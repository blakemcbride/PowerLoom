  ...
  // Should be synchronized on process lock oBOOTSTRAP_LOCKo
  {
    ...
    { 
      ...
      if (currentStartupTimePhaseP(7)) {
        defineFunctionObject("STARTUP-RDBMS-SYSTEM", "(DEFUN STARTUP-RDBMS-SYSTEM () :PUBLIC? TRUE :C-CALLABLE? TRUE :C-NAMESPACE-PREFIX \"stella_\")", ((cpp_function_code)(&startupRdbmsSystem)), NULL);
...
extern "C" void stella_startupRdbmsSystem() {
  // C-callable wrapper for `startupRdbmsSystem'.
  startupRdbmsSystem();
}

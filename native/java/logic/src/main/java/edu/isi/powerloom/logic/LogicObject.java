...
public class LogicObject extends ContextSensitiveObject {
    /** List of direct parents whose <code>variableValue</code> slot points to this object.
     */
  ...
  public static Cons allSlotValueTypes(LogicObject self, Surrogate relation) {
    return (Logic.mostSpecificNamedDescriptions(Cons.copyConsList(Logic.currentInferenceLevel().levellizedAllSlotValueTypes(self, relation))));
  }
...
;
  ...
  public static Stella_Object accessLogicObjectSlotValue(LogicObject self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    if (slotname == Logic.SYM_LOGIC_VARIABLE_VALUE_INVERSE) {
      if (setvalueP) {

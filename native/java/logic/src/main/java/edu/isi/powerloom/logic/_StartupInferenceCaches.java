    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(2)) {
          ...
          Logic.KWD_DISABLE_JITp = ((Keyword)(GeneralizedSymbol.internRigidSymbolWrtModule("DISABLE-JIT?", null, 2)));
          Logic.KWD_JUST_IN_TIME_INFERENCE = ((Keyword)(GeneralizedSymbol.internRigidSymbolWrtModule("JUST-IN-TIME-INFERENCE", null, 2)));
        ...
        if (Stella.currentStartupTimePhaseP(7)) {
          ...
          Stella.defineFunctionObject("CALL-PROPAGATE-CONSTRAINTS", "(DEFUN CALL-PROPAGATE-CONSTRAINTS ((CONTEXT CONTEXT)) :DOCUMENTATION \"OBSOLETE: use `call-run-forward-rules' instead.\")", Native.find_java_method("edu.isi.powerloom.logic.Logic", "callPropagateConstraints", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Context")}), null);
          Stella.defineFunctionObject("PROPAGATE-CONSTRAINTS", "(DEFUN PROPAGATE-CONSTRAINTS (|&REST| (NAME NAME)) :DOCUMENTATION \"OBSOLETE: use `run-forward-rules' instead.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)", Native.find_java_method("edu.isi.powerloom.logic.Logic", "propagateConstraints", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}), Native.find_java_method("edu.isi.powerloom.logic.Logic", "propagateConstraintsEvaluatorWrapper", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}));
          Stella.defineFunctionObject("REPROPAGATE-CONSTRAINTS", "(DEFUN REPROPAGATE-CONSTRAINTS (|&REST| (NAME NAME)) :DOCUMENTATION \"OBSOLETE: use `run-forward-rules' instead.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)", Native.find_java_method("edu.isi.powerloom.logic.Logic", "repropagateConstraints", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}), Native.find_java_method("edu.isi.powerloom.logic.Logic", "repropagateConstraintsEvaluatorWrapper", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}));
          Stella.defineFunctionObject("CALL-RUN-FORWARD-RULES", "(DEFUN CALL-RUN-FORWARD-RULES ((CONTEXT CONTEXT) (LOCAL? BOOLEAN) (FORCE? BOOLEAN)) :DOCUMENTATION \"Callable implementation for `run-forward-rules' (which see).\" :PUBLIC? TRUE)", Native.find_java_method("edu.isi.powerloom.logic.Logic", "callRunForwardRules", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Context"), java.lang.Boolean.TYPE, java.lang.Boolean.TYPE}), null);
          Stella.defineFunctionObject("RUN-FORWARD-RULES", "(DEFUN RUN-FORWARD-RULES (|&REST| (OPTIONS OBJECT)) :DOCUMENTATION \"Run forward inference rules in the module defined by the :module option (which\ndefaults to the current module).  If forward inferencing is already up-to-date\nin the designated module, no additional inferencing will occur, unless the :force?\noption is specified as TRUE, in which case all forward rules are run or rerun (for\nbackwards compatibility, this command also supports the old <module> :force arguments\nspecified with a non-standard keyword notation).  If :local? is TRUE (the default),\nonly propositions asserted locally in the module will be posted for forward inference,\notherwise, all true propositions visible in the module will be posted.\n\nCalling `run-forward-rules' with :disable-jit? TRUE will disable future just-in-time\nforward inference which will preserve cached forward inference in case of monotonic\nassertions that do not clip (implicitly retract) any existing values.  Subsequent calls\ncan then trigger new inferences based on these updates only.  If the JIT stays enabled,\neven monotonic updates will throw away the JIT cache.  Also, once any retractions have\nbeen performed, all cached inferences will be thrown away and forward inference has to\nbe rerun from scratch or just-in-time forward inference needs to be enabled again.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)", Native.find_java_method("edu.isi.powerloom.logic.Logic", "runForwardRules", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}), Native.find_java_method("edu.isi.powerloom.logic.Logic", "runForwardRulesEvaluatorWrapper", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}));
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFSPECIAL *INVISIBLEASSERTION?* BOOLEAN FALSE :DOCUMENTATION \"Used to signal an assertion that doesn't\nblow away inference caches.\")", null);
        }

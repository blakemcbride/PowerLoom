    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(2)) {
          ...
          Logic.SYM_PL_KERNEL_KB_SKOLEM_FN = ((Symbol)(GeneralizedSymbol.internRigidSymbolWrtModule("SKOLEM-FN", Stella.getStellaModule("/PL-KERNEL-KB", true), 0)));
        ...
        if (Stella.currentStartupTimePhaseP(7)) {
          ...
          Stella.defineFunctionObject("SKOLEMIZE-RULE", "(DEFUN SKOLEMIZE-RULE ((RULE PROPOSITION)))", Native.find_java_method("edu.isi.powerloom.logic.Proposition", "skolemizeRule", new java.lang.Class [] {Native.find_java_class("edu.isi.powerloom.logic.Proposition")}), null);
          Stella.defineFunctionObject("SKOLEMIZE-RULE-FOR-GOAL", "(DEFUN (SKOLEMIZE-RULE-FOR-GOAL CONS) ((GOAL CONS) (ANTECEDENT CONS) (FORALLVARS (CONS OF SYMBOL)) (EXISTSVARS (CONS OF SYMBOL)) (SKOLEMFNID SYMBOL)))", Native.find_java_method("edu.isi.powerloom.logic.Logic", "skolemizeRuleForGoal", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons"), Native.find_java_class("edu.isi.stella.Cons"), Native.find_java_class("edu.isi.stella.Cons"), Native.find_java_class("edu.isi.stella.Cons"), Native.find_java_class("edu.isi.stella.Symbol")}), null);
          ...
          Stella.defineFunctionObject("PROPOSITION-HAS-STRENGTH?", "(DEFUN (PROPOSITION-HAS-STRENGTH? BOOLEAN) ((SELF PROPOSITION) (MINIMALSTRENGTH TRUTH-VALUE)) :DOCUMENTATION \"Return true if the truth-value of `self' satisfies `minimalStrength'.\" :PUBLIC? TRUE)", Native.find_java_method("edu.isi.powerloom.logic.Proposition", "propositionHasStrengthP", new java.lang.Class [] {Native.find_java_class("edu.isi.powerloom.logic.Proposition"), Native.find_java_class("edu.isi.powerloom.logic.TruthValue")}), null);
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFGLOBAL *LAZY-SATELLITE-RULES?* BOOLEAN FALSE :DOCUMENTATION \"If true, inversion of forall propositions happens lazily.\nI.e., satellite rules are generated on-demand during inference only if they\nare actually needed.\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *DERIVE-DEFERRED-SATELLITE-RULES-INVOCATIONS* (LIST OF DESCRIPTION) NULL)", null);
        }

...
public class TrainingExample extends StandardObject {
  ...
  /** Variant of <code>createCachedNetwork</code> that takes a training <code>example</code>,
   * runs its cons query, and stores a compacted version of the associated proof tree
   * as the <code>example</code>s cached solution which will also be returned.  If a cached and
   * up-to-date solution already exists, it will be returned instead.
   * @param example
   * @return Justification
   */
  public static Justification getCachedNetworkProof(TrainingExample example) {
    if ((example.cachedSolution != null) &&
        (example.timestamp == Logic.getNowTimestamp())) {
        ...
        { Module module = ((temp000 != null) ? temp000 : ((Module)(Stella.$MODULE$.get())));
          ...
          { Object old$Module$000 = Stella.$MODULE$.get();
            ...
            try {
              ...
              { QueryIterator queryiter = Logic.createAskQuery(Cons.consList(Cons.cons(example.query, Cons.cons(Logic.KWD_RECORD_JUSTIFICATIONSp, Cons.cons(Stella.TRUE_WRAPPER, Cons.cons(Logic.KWD_MATCH_MODE, Cons.cons(Logic.KWD_CHAMELEON, Stella.NIL)))))));

                Logic.callAskPartial(queryiter);
                example.cachedSolution = ((Justification)(KeyValueList.dynamicSlotValue(queryiter.baseControlFrame.dynamicSlots, Logic.SYM_LOGIC_JUSTIFICATION, null)));
                if (example.cachedSolution == null) {
                  { Object old$PrintreadablyP$000 = Stella.$PRINTREADABLYp$.get();

                    try {
                      Native.setBooleanSpecial(Stella.$PRINTREADABLYp$, true);
                      Stella.STANDARD_WARNING.nativeStream.println("WARNING: get-cached-network-proof: training example query failed: " + example.query);
                  ...
                  ControlFrame.recordFailJustification(queryiter.baseControlFrame, Logic.KWD_UP_FAIL);
                  example.cachedSolution = ((Justification)(KeyValueList.dynamicSlotValue(queryiter.baseControlFrame.dynamicSlots, Logic.SYM_LOGIC_JUSTIFICATION, null)));
                }
                example.cachedSolution = Justification.compactPartialProofToNetworkProof(((Justification)(example.cachedSolution)));
                example.timestamp = Logic.getNowTimestamp();
                return (((Justification)(example.cachedSolution)));
              }

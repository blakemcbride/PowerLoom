  ...
  static void helpStartupKifOut3() {
    {
      ...
      Stella.defineMethodObject("(DEFMETHOD DESCRIBE-OBJECT ((SELF NAMED-DESCRIPTION) (STREAM OUTPUT-STREAM) (MODE KEYWORD)) :DOCUMENTATION \"Prints a description of 'self' to stream 'stream'.  'mode'\ncan be :terse, :verbose, or :source.  Used by `describe'.\" :PUBLIC? TRUE)", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.NamedDescription", "describeObject", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.OutputStream"), Native.find_java_class("edu.isi.stella.Keyword")})), null);
      ...
      Stella.defineMethodObject("(DEFMETHOD STORE-NATIVE-RELATION ((STORE OBJECT-STORE) (RELATION RELATION)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "storeNativeRelation", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Relation")})), null);
      Stella.defineMethodObject("(DEFMETHOD STORE-RELATION ((STORE OBJECT-STORE) (RELATION NAMED-DESCRIPTION)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "storeRelation", new java.lang.Class [] {Native.find_java_class("edu.isi.powerloom.logic.NamedDescription")})), null);
      Stella.defineMethodObject("(DEFMETHOD STORE-ASSERTION ((STORE OBJECT-STORE) (PROPOSITION PROPOSITION)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "storeAssertion", new java.lang.Class [] {Native.find_java_class("edu.isi.powerloom.logic.Proposition")})), null);
      Stella.defineMethodObject("(DEFMETHOD STORE-MODULE-HEADER ((STORE OBJECT-STORE) (MODULE MODULE)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "storeModuleHeader", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Module")})), null);
      Stella.defineMethodObject("(DEFMETHOD STORE-MODULE-TRAILER ((STORE OBJECT-STORE) (MODULE MODULE)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "storeModuleTrailer", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Module")})), null);
      Stella.defineMethodObject("(DEFMETHOD CLEAR-OBJECT-STORE ((STORE OBJECT-STORE)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.powerloom.logic.ObjectStore", "clearObjectStore", new java.lang.Class [] {})), null);
    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRETTYPRINTLOGICALFORMS?* BOOLEAN FALSE :PUBLIC? TRUE :DOCUMENTATION \"Controls whether logical forms print on single lines\n(unformatted) or multi-line indented.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRETTYPRINTKIF?* BOOLEAN FALSE :PUBLIC? TRUE :DOCUMENTATION \"Controls whether KIF expressions print on single lines\n(unformatted) or multi-line indented.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRINTLOGICALFORMSTREAM* OUTPUT-STREAM NULL :PUBLIC? TRUE :DOCUMENTATION \"Eliminates necessity of passing stream argument\nthroughout 'print-logical-form' functions.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *DOWNCASEOPERATORS?* BOOLEAN FALSE :PUBLIC? TRUE :DOCUMENTATION \"Controls whether down-casing happens during logical\nform printing (leading to lots more string garbage).\")", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *LOGIC-DIALECT-PRINT-FUNCTIONS* (KEY-VALUE-LIST OF KEYWORD FUNCTION-CODE-WRAPPER) (NEW (KEY-VALUE-LIST OF KEYWORD FUNCTION-CODE-WRAPPER)) :DOCUMENTATION \"Table to allow extension of the logical form printing code\nby associating keywords with print functions.  Should have values added only\nvia the associated function REGISTER-LOGIC-DIALECT-PRINT-FUNCTION.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRINTQUANTIFIEDOBJECTSSTACK* CONS NULL :DOCUMENTATION \"Used to register objects that quantify variables, so we\ncan decide in certain contexts whether to print a variable or its value.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *INDENTCOUNTER* INTEGER 0 :PUBLIC? TRUE :DOCUMENTATION \"The number of spaces prepended during printing a logical form.\")", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *INDENT-QUANTUM* INTEGER 3 :PUBLIC? TRUE :DOCUMENTATION \"The number of spaces added by a call to 'increase-indent'.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *TOPLEVELPRINTKIFPROPOSITION?* BOOLEAN TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *EXCLUDEDPROPOSITIONS* (HASH-TABLE OF PROPOSITION BOOLEAN-WRAPPER) NULL :PUBLIC? TRUE :DOCUMENTATION \"List of propositions that shouldn't be saved by `save-module'.\")", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *BUILT-IN-MODULE-NAMES* (CONS OF STRING-WRAPPER) (QUOTE (\"PL-USER\" \"PL-KERNEL-KB\" \"PLI\" \"LOOM-API\" \"CYC-FRAME-ONTOLOGY\" \"KIF-FRAME-ONTOLOGY\" \"IDL-THEORY\" \"IDL-USER-THEORY\" \"LOGIC\" \"STELLA\" \"COMMON-LISP\")))", null);
        }

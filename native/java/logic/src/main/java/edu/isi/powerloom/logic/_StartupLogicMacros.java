...
public class _StartupLogicMacros {
  static void helpStartupLogicMacros1() {
    {
      { Module stellamodule = Stella.$STELLA_MODULE$;
        Module logicmodule = Stella.getStellaModule("LOGIC", true);
        Module plkernelmodule = Stella.getStellaModule("PL-KERNEL", true);

        Surrogate.internSurrogateInModule("PROPOSITION.IF", stellamodule, true);
        Surrogate.internSurrogateInModule("=", stellamodule, true);
        Symbol.importSymbol(Symbol.lookupSymbolInModule("=", stellamodule, true), plkernelmodule);
        Surrogate.importSurrogate(Surrogate.lookupSurrogateInModule("=", stellamodule, true), plkernelmodule);
        { Stella_Object name = null;
          Cons iter001 = Stella.getQuotedTree("((\"LISTOF\" \"SETOF\") \"/LOGIC\")", "/LOGIC");

          for (;!(iter001 == Stella.NIL); iter001 = iter001.rest) {
            name = iter001.value;
            { StringWrapper sname = ((StringWrapper)(name));
              Symbol plkernelsym = Symbol.internSymbolInModule(sname.wrapperValue, plkernelmodule, true);
              Surrogate plkernelsgt = Surrogate.internSurrogateInModule(sname.wrapperValue, plkernelmodule, true);

              Symbol.importSymbol(plkernelsym, logicmodule);
              Surrogate.importSurrogate(plkernelsgt, logicmodule);
            }
          }
        }
      }
  ...
  static void helpStartupLogicMacros2() {
    {
      ...
      Logic.SYM_STELLA_FIRST_DEFINED = ((Symbol)(GeneralizedSymbol.internRigidSymbolWrtModule("FIRST-DEFINED", Stella.getStellaModule("/STELLA", true), 0)));
    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(2)) {
          ...
          Logic.SYM_LOGIC_$DATA_FINDER_KLUDGES_ENABLEDp$ = ((Symbol)(GeneralizedSymbol.internRigidSymbolWrtModule("*DATA-FINDER-KLUDGES-ENABLED?*", null, 0)));
        ...
        if (Stella.currentStartupTimePhaseP(7)) {
          ...
          Stella.defineFunctionObject("WHEN-DATA-FINDER-KLUDGES-ENABLED", "(DEFUN WHEN-DATA-FINDER-KLUDGES-ENABLED (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)", Native.find_java_method("edu.isi.powerloom.logic.Logic", "whenDataFinderKludgesEnabled", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.Cons")}), null);
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFSPECIAL *SUPPRESSNONLOGICOBJECTWARNING?* BOOLEAN TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *CYC-KLUDGES-ENABLED?* BOOLEAN FALSE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *DATA-FINDER-KLUDGES-ENABLED?* BOOLEAN FALSE)", null);
        }

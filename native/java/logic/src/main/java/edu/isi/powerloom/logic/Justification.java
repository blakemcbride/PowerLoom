...
public class Justification extends StandardObject {
    ...
    /** Error value used by partial matcher
     */
  ...
  public static Justification newJustification() {
    { Justification self = null;
      ...
      self.errorScore = Stella.NULL_FLOAT;
      return (self);
    }
  }
...
;
   ...
   * @param error
   */
  public static void backwardPropagateCachedNetworkMultiProof(Justification proof, double error) {
    if (Logic.$RULE_COMBINATION$ == Logic.KWD_MAX) {
      { double max = Stella.MOST_NEGATIVE_FLOAT;
        ...
        Justification.backwardPropagateCachedNetworkProof(proof, error);
      }
    }
      ...
      { Justification ant = null;
        ...
        for (;!(iter001 == Stella.NIL); iter001 = iter001.rest) {
          ...
          Justification.backwardPropagateCachedNetworkProof(ant, error * ant.matchScore);
        }
      }
    }
    else {
  ...
  /** Propagate the <code>error</code> between <code>proof</code>s conclusion and the desired target value
   * through <code>proof</code>s network and its antecedents, and adjust weights to decrease the error.  Multiple
   * iterations through <code>forwardSbackwardPropagateCachedNetworkProof</code> with updated top-level
   * errors will train the involved networks to minimize the error as much as possible.
   * @param proof
   * @param error
   */
  public static void backwardPropagateCachedNetworkProof(Justification proof, double error) {
    { Keyword testValue000 = proof.inferenceRule;

      if (testValue000 == Logic.KWD_MULTIPLE_PROOFS) {
        Justification.backwardPropagateCachedNetworkMultiProof(proof, error);
      }
      else if ((testValue000 == Logic.KWD_DISPROOF) ||
          (testValue000 == Logic.KWD_GOAL_COMPLEMENT)) {
        Justification.backwardPropagateCachedNetworkProof(((Justification)(proof.antecedents.value)), error);
      }
        ...
        { NeuralNetwork net = Justification.getJustificationNeuralNetwork(proof);
          ...
          Justification.forwardPropagateCachedNetworkFromJustification(proof);
            ...
            for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
              ...
              if (!(ant.inferenceRule == Logic.KWD_PRIMITIVE_STRATEGY)) {
                { int index = net.truthValueArgumentIndex(ant.proposition);
                  ...
                  if (index >= 0) {
                    Justification.backwardPropagateCachedNetworkProof(ant, net.nthInputError(index));
                    reactivateP = true;
                  }
                }
              }
  ...
  /** Locally forward-propagate the network associated with <code>just</code> based on
   * previously cached <code>positiveScore</code>s of antecedents.
   * @param just
   * @return double
   */
  public static double forwardPropagateCachedNetworkFromJustification(Justification just) {
    { double score = 0.0;
      ...
      { Keyword testValue000 = just.inferenceRule;
        ...
        if ((testValue000 == Logic.KWD_AND_INTRODUCTION) ||
            (testValue000 == Logic.KWD_OR_INTRODUCTION)) {
          { NeuralNetwork net = Justification.getJustificationNeuralNetwork(just);
   ...
   * @return double
   */
  public static double forwardPropagateCachedNetworkProof(Justification proof) {
    { double score = 0.0;
          ...
          { NeuralNetwork net = Justification.getJustificationNeuralNetwork(proof);
            ...
            { Justification ant = null;
              ...
              for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
                ...
                index = net.truthValueArgumentIndex(ant.proposition);
                if (index >= 0) {
                  score = Justification.forwardPropagateCachedNetworkProof(ant);
                  (inputs.theArray)[index] = (FloatWrapper.wrapFloat(score));
                }
                if (vectorargs != null) {
                  index = net.vectorArgumentIndex(ant.proposition);
          ...
          { Cons value000 = Stella.NIL;
            ...
            { Justification ant = null;
              ...
              for (;!(iter001 == Stella.NIL); iter001 = iter001.rest) {
                ...
                if (collect000 == null) {
                  {
                    collect000 = Cons.cons(FloatWrapper.wrapFloat(Justification.forwardPropagateCachedNetworkProof(ant)), Stella.NIL);
                  ...
                  {
                    collect000.rest = Cons.cons(FloatWrapper.wrapFloat(Justification.forwardPropagateCachedNetworkProof(ant)), Stella.NIL);
  ...
  /** Convert <code>proof</code> into a compacted network tree form that only contains
   * :AND, :OR, :MULTI and :PRIMITIVE nodes (some of which such as <code>instanceOf</code> might contain
   * further antecedents if they were computed by specialists, for example).
   * @param proof
   * @return Justification
   */
  public static Justification compactPartialProofToNetworkProof(Justification proof) {
    { Keyword testValue000 = proof.inferenceRule;

      if (testValue000 == Logic.KWD_PRIMITIVE_STRATEGY) {
        if (((PrimitiveStrategy)(proof)).strategy == Logic.KWD_GOAL_COMPLEMENT) {
          proof = proof.shallowCopy();
          proof.inferenceRule = Logic.KWD_GOAL_COMPLEMENT;
          return (Justification.compactPartialProofToNetworkProof(proof));
        }
        else if (proof.antecedents == Stella.NIL) {
          return (proof);
        }
        else {
          { OutputStringStream stream000 = OutputStringStream.newOutputStringStream();

            stream000.nativeStream.print("INTERNAL ERROR: unexpected primitive justifications with antecedents for: `" + proof.proposition + "'");
            throw ((StellaException)(StellaException.newStellaException(stream000.theStringReader()).fillInStackTrace()));
          }
        }
      }
        ...
        { Cons value000 = Stella.NIL;
          ...
          { Justification ant = null;
            ...
            for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
              ...
              if (collect000 == null) {
                {
                  collect000 = Cons.cons(Justification.compactPartialProofToNetworkProof(ant), Stella.NIL);
                  if (value000 == Stella.NIL) {
                    value000 = collect000;
                  }
                  else {
                ...
                {
                  collect000.rest = Cons.cons(Justification.compactPartialProofToNetworkProof(ant), Stella.NIL);
                  collect000 = collect000.rest;
                }
              }
            }
      ...
      else if ((testValue000 == Logic.KWD_MODUS_PONENS) ||
          (testValue000 == Logic.KWD_MODUS_TOLLENS)) {
        { Stella_Object relation = ((Surrogate)(proof.proposition.operator)).surrogateValue;

          if ((relation != null) &&
              (Logic.testPropertyP(relation, Logic.SGT_CHAMELEON_TRUTH_VALUE_RELATION) ||
               (!(Logic.testPropertyP(relation, Logic.SGT_CHAMELEON_IGNORED_VALUE_RELATION) ||
              (Logic.testPropertyP(relation, Logic.SGT_CHAMELEON_VECTOR_RELATION) &&
               (!Logic.testPropertyP(relation, Logic.SGT_CHAMELEON_TRUTH_VALUE_RELATION))))))) {
            proof = proof.shallowCopy();
            proof.inferenceRule = Logic.KWD_MULTIPLE_PROOFS;
            proof.antecedents = Cons.cons(Justification.compactPartialProofToNetworkProof(((Justification)(proof.antecedents.rest.value))), Stella.NIL);
            return (proof);
          }
          else {
            proof = proof.shallowCopy();
            proof.inferenceRule = Logic.KWD_PRIMITIVE_STRATEGY;
            proof.antecedents = Stella.NIL;
            return (proof);
          }
        }
      }
      else if (testValue000 == Logic.KWD_SUBSUMPTION_REASONING) {
        proof = proof.shallowCopy();
        proof.inferenceRule = Logic.KWD_PRIMITIVE_STRATEGY;
        proof.antecedents = Stella.NIL;
        return (proof);
      }
      else if (testValue000 == Logic.KWD_FAIL_INTRODUCTION) {
        proof = proof.shallowCopy();
        proof.inferenceRule = Logic.KWD_PRIMITIVE_STRATEGY;
        proof.antecedents = Stella.NIL;
        return (proof);
        ...
        { Stella_Object relation = ((Surrogate)(proof.proposition.operator)).surrogateValue;
          ...
          if ((relation != null) &&
              Logic.testPropertyP(relation, Logic.SGT_CHAMELEON_PRIMITIVE_VALUE_RELATION)) {
            proof = proof.shallowCopy();
            proof.inferenceRule = Logic.KWD_PRIMITIVE_STRATEGY;
            proof.antecedents = Stella.NIL;
            return (proof);
            ...
            switch (proof.antecedents.length()) {
              case 0: 
                return (proof);
              case 1: 
                return (Justification.compactPartialProofToNetworkProof(((Justification)(proof.antecedents.value))));
              default:
                { OutputStringStream stream001 = OutputStringStream.newOutputStringStream();

                  stream001.nativeStream.print("Unhandled network proof justification with multiple antecedents: `" + proof.inferenceRule + "'");
                  throw ((StellaException)(StellaException.newStellaException(stream001.theStringReader()).fillInStackTrace()));
                }
            }
          }
   ...
   */
  public static NeuralNetwork getJustificationNeuralNetwork(Justification just) {
    return (Proposition.getPropositionNeuralNetwork(just.proposition, true));
  }
    ...
    { Object old$ExplanationStyle$000 = Logic.$EXPLANATION_STYLE$.get();
      ...
      try {
        ...
        { Cons antecedents = ((style == Logic.KWD_RAW) ? self.antecedents : Justification.visibleAntecedents(self).theConsList);
          ...
          if (self.matchScore != Stella.NULL_FLOAT) {
            keys.insertAt(Logic.KWD_MATCH_SCORE, FloatWrapper.wrapFloat(self.matchScore));
          }
  ...
  public static Stella_Object accessJustificationSlotValue(Justification self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    ...
    else if (slotname == Logic.SYM_LOGIC_ERROR_SCORE) {
      if (setvalueP) {
        self.errorScore = ((FloatWrapper)(value)).wrapperValue;
      }
      else {
        value = FloatWrapper.wrapFloat(self.errorScore);
      }
    }
    else {
      { OutputStringStream stream000 = OutputStringStream.newOutputStringStream();

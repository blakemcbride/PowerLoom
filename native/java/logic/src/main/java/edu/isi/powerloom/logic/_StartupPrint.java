    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFGLOBAL *DEBUG-PRINT-MODE?* BOOLEAN FALSE :DOCUMENTATION \"If set to TRUE, modify printing so that it\ndoesn't break and is more informative during a debugging session.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRINTMODE* KEYWORD :FLAT :DOCUMENTATION \"Controls the kind of detail that gets printed about\nindividual objects.  Values are :ORIGINAL, :REALISTIC, :FLAT, :DEBUG-LOW, and\n :DEBUG-HIGH.\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *PRINTFUNCTIONSASRELATIONS?* BOOLEAN FALSE :DOCUMENTATION \"Controls whether functions are printed as relations (F i v) or\nas functions (= (F i) v).\")", null);
          Stella.defineGlobalVariableObject("(DEFCONSTANT VARIABLE-BINDING-SEPARATOR STRING \"/\")", null);
          Stella.defineGlobalVariableObject("(DEFCONSTANT VALUE-OF-SEPARATOR STRING \"//\")", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *SKOLEMNAMEMAPPINGTABLE* (ENTITY-MAPPING OF SKOLEM SKOLEM) NULL :DOCUMENTATION \"If set, maps some skolems to names other than\ntheir own.\")", null);
        }

  ...
  static void helpStartupSdbc4() {
    {
      ...
      Stella.defineMethodObject("(DEFMETHOD (NEXT? BOOLEAN) ((SELF RESULT-SET-ITERATOR)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.sdbc.ResultSetIterator", "nextP", new java.lang.Class [] {})), null);
    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFGLOBAL *DB-CONNECTION-RENEWAL-INTERVAL* INTEGER 60 :DOCUMENTATION \"The maximum lifetime of a database connection (in seconds) after which\nit gets automatically renewed.  Auto-renewals avoid connection timeout errors which are\notherwise tricky to catch, e.g., we might just see a `bad handle passed' error.\" :DEMON-PROPERTY \"sdbc.dbConnectionRenewalInterval\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFSPECIAL *SQL-COMMAND-TEMPLATES* (KEY-VALUE-MAP OF OBJECT (KEY-VALUE-LIST OF OBJECT PROPERTY-LIST)) (NEW KEY-VALUE-MAP))", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *SQL-CMD-TEMPLATE-VARIABLE-PREFIX* STRING \"#$\")", null);
        }

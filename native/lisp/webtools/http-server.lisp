...
(CL:DEFVAR SYM-HTTP-SERVER-HTTP-SET-HTTP-DOCUMENT-ROOT NULL)
...
(CL:DEFVAR SYM-HTTP-SERVER-HTTP-PUBLISH-FILE NULL)
(CL:DEFVAR SYM-HTTP-SERVER-HTTP-PUBLISH-DIRECTORY NULL)
...
(CL:DEFUN GET-HTTP-DOCUMENT-ROOT-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:SETQ ARGUMENTS ARGUMENTS)
  (CL:LET* ((RESULT (GET-HTTP-DOCUMENT-ROOT)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING RESULT))
   (CL:IF (CL:NOT (CL:EQ RESULT STELLA::NULL-STRING))
    (WRAP-STRING RESULT) NULL)))
...
(CL:DEFUN SET-HTTP-DOCUMENT-ROOT-EVALUATOR-WRAPPER (ARGUMENTS)
  (%SET-HTTP-DOCUMENT-ROOT (%WRAPPER-VALUE (%%VALUE ARGUMENTS))))

(CL:DEFMACRO SET-HTTP-DOCUMENT-ROOT (CL:&WHOLE EXPRESSION CL:&REST IGNORE)
  "Set the document root to `root'.  Important: this needs to be
called before any handlers are published, otherwise it won't be effective."
  (CL:DECLARE (CL:IGNORE IGNORE))
  (CL:LET ((*IGNORETRANSLATIONERRORS?* FALSE))
...
(CL:SETF (CL:MACRO-FUNCTION (CL:QUOTE |/HTTP/SET-HTTP-DOCUMENT-ROOT|)) (CL:MACRO-FUNCTION (CL:QUOTE SET-HTTP-DOCUMENT-ROOT)))
...
(CL:DEFUN PRINT-REGISTERED-HTTP-HANDLERS ()
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Document root: "
   (GET-HTTP-DOCUMENT-ROOT) EOL EOL)
  (CL:LET* ((PATHS NIL))
   (CL:LET*
    ((PATH NULL) (OPTIONS NULL)
     (ITER-000 (ALLOCATE-ITERATOR *HTTP-HANDLER-REGISTRY*)))
...
(CL:DEFUN %PUBLISH-FILE (PATH FILE OPTIONS)
...
(CL:DEFUN PUBLISH-FILE-EVALUATOR-WRAPPER (ARGUMENTS)
  (%PUBLISH-FILE (%WRAPPER-VALUE (%%VALUE ARGUMENTS))
   (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS)))
   (%%REST (%%REST ARGUMENTS))))

(CL:DEFMACRO PUBLISH-FILE (CL:&WHOLE EXPRESSION CL:&REST IGNORE)
  "Publish `path' to be served with `file' which must exist and be readable.
If :content-type is supplied, it will be used when serving `file', otherwise, a content
type is guessed from `file's extension."
  (CL:DECLARE (CL:IGNORE IGNORE))
...
(CL:SETF (CL:MACRO-FUNCTION (CL:QUOTE |/HTTP/PUBLISH-FILE|)) (CL:MACRO-FUNCTION (CL:QUOTE PUBLISH-FILE)))
...
(CL:DEFUN %PUBLISH-DIRECTORY (PATH DIRECTORY OPTIONS)
...
(CL:DEFUN PUBLISH-DIRECTORY-EVALUATOR-WRAPPER (ARGUMENTS)
  (%PUBLISH-DIRECTORY (%WRAPPER-VALUE (%%VALUE ARGUMENTS))
   (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS)))
   (%%REST (%%REST ARGUMENTS))))

(CL:DEFMACRO PUBLISH-DIRECTORY (CL:&WHOLE EXPRESSION CL:&REST IGNORE)
  "Make all files descending from `directory' available on the server.
`path' will be coerced to end in a delimiter and `directory' must exist.
This is very rudimentary right now and does not provide any sophisticated access
control.  It recurses into subdirectories of `directory', prevents escape via `..'
...
(CL:SETF (CL:MACRO-FUNCTION (CL:QUOTE |/HTTP/PUBLISH-DIRECTORY|)) (CL:MACRO-FUNCTION (CL:QUOTE PUBLISH-DIRECTORY)))
...
(CL:DEFUN HELP-STARTUP-HTTP-SERVER1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-HTTP-SERVER-HTTP-SET-HTTP-DOCUMENT-ROOT
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-HTTP-DOCUMENT-ROOT" NULL 0))
   ...
   (CL:SETQ SYM-HTTP-SERVER-HTTP-PUBLISH-FILE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PUBLISH-FILE" NULL 0))
   (CL:SETQ SYM-HTTP-SERVER-HTTP-PUBLISH-DIRECTORY
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PUBLISH-DIRECTORY" NULL 0))
...
address of the started server instance.\" :PUBLIC? TRUE :COMMAND? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION START-HTTP-SERVER-IMPL)) NULL)
...
server instance running, this is a no-op.\" :PUBLIC? TRUE :COMMAND? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION STOP-HTTP-SERVER-IMPL)) NULL)
...
If there are multiple values, an arbitrary one will be returned.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-HEADER-VALUE-IMPL)) NULL)
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION SET-REPLY-HEADER-VALUE-IMPL)) NULL)
...
will return /foo/bar.html#frag?a+b+c .\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-URI-IMPL)) NULL)
...
return a+b+c .\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI-QUERY)
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-URI-QUERY-IMPL)) NULL)
...
return /foo/bar.html.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-URI-PATH-IMPL)) NULL)
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-METHOD-IMPL)) NULL)
...
if this was a GET), the result will be the empty string.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-BODY-IMPL)) NULL)
...
Note that different implementations have slightly different behavior.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-LOCAL-ADDRESS-IMPL))
    NULL)
...
Note that different implementations have slightly different behavior.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REQUEST-REMOTE-ADDRESS-IMPL))
    NULL)
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-REPLY-STREAM-IMPL)) NULL)
...
:OK (code 200) is used.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION SET-RESPONSE-CODE-IMPL)) NULL)
...
standard `lookup' calls will return the value of the first key only.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-DOCUMENT-ROOT"
    "(DEFUN (GET-HTTP-DOCUMENT-ROOT STRING) () :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION GET-HTTP-DOCUMENT-ROOT)
    (CL:FUNCTION GET-HTTP-DOCUMENT-ROOT-EVALUATOR-WRAPPER))
...
called before any handlers are published, otherwise it won't be effective.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION %SET-HTTP-DOCUMENT-ROOT)
    (CL:FUNCTION SET-HTTP-DOCUMENT-ROOT-EVALUATOR-WRAPPER))
   (DEFINE-METHOD-OBJECT
...
handler whose path is identical to `path' or is the longest prefix of `path'.\" :PUBLIC? TRUE)"
    (WRAP-METHOD-CODE (CL:FUNCTION LOOKUP-HANDLER)) NULL)
...
If :content-type is supplied, it will be used when serving `file', otherwise, a content
type is guessed from `file's extension.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION %PUBLISH-FILE)
    (CL:FUNCTION PUBLISH-FILE-EVALUATOR-WRAPPER))
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD PUBLISH-FILE-IMPL ((SERVER HTTP-SERVER) (PATH STRING) (FILE STRING) (OPTIONS PROPERTY-LIST)))"
    (WRAP-METHOD-CODE (CL:FUNCTION PUBLISH-FILE-IMPL)) NULL)
...
This is very rudimentary right now and does not provide any sophisticated access
control.  It recurses into subdirectories of `directory', prevents escape via `..'
and does not (yet) support directory listings.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION %PUBLISH-DIRECTORY)
    (CL:FUNCTION PUBLISH-DIRECTORY-EVALUATOR-WRAPPER))
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD PUBLISH-DIRECTORY-IMPL ((SERVER HTTP-SERVER) (PATH STRING) (DIRECTORY STRING) (OPTIONS PROPERTY-LIST)))"
    (WRAP-METHOD-CODE (CL:FUNCTION PUBLISH-DIRECTORY-IMPL)) NULL)
...
the content type of the resulting page, otherwise, text/html will be used by default.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION PUBLISH-HANDLER-IMPL)) NULL)
...
This includes required as well as optional arguments.  This will never return NULL.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION GET-HANDLER-OPTIONS-IMPL)) NULL)
...
with an actual native server object.\")" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *DEFAULT-HTTP-SERVER-PORT* INTEGER 9090)" NULL)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     ...
     NULL)
    (REGISTER-NATIVE-NAME SYM-HTTP-SERVER-HTTP-SET-HTTP-DOCUMENT-ROOT
     KWD-HTTP-SERVER-COMMON-LISP KWD-HTTP-SERVER-FUNCTION)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *HTTP-HANDLER-REGISTRY* (KEY-VALUE-MAP OF STRING-WRAPPER PROPERTY-LIST) (NEW KEY-VALUE-MAP))"
     NULL)
    (REGISTER-NATIVE-NAME SYM-HTTP-SERVER-HTTP-PUBLISH-FILE
     KWD-HTTP-SERVER-COMMON-LISP KWD-HTTP-SERVER-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-HTTP-SERVER-HTTP-PUBLISH-DIRECTORY
     KWD-HTTP-SERVER-COMMON-LISP KWD-HTTP-SERVER-FUNCTION)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
    ...
    (%PUBLISH-DIRECTORY "/ploom/" (GET-HTTP-DOCUMENT-ROOT)
     (CONS-LIST KWD-HTTP-SERVER-DOCUMENTATION

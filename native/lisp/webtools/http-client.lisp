  ...
  (CL:LET* ((OUT NULL))
   (CL:UNWIND-PROTECT
    (CL:PROGN (CL:SETQ OUT (OPEN-OUTPUT-FILE FILE))
     (CL:LET* ((IN NULL))
      (CL:UNWIND-PROTECT
       (CL:PROGN (CL:SETQ IN (OPEN-URL-STREAM URL))
        (COPY-STREAM-TO-STREAM IN OUT))
       (CL:WHEN (CL:NOT (CL:EQ IN NULL)) (CLOSE-STREAM IN)))))
...
CAUTION: String conversion will not do the right thing for binary data in Java."
  ...
  #+MCL
  (CL:CHECK-TYPE URL CL:SIMPLE-STRING)
  (CL:LET* ((RESULT STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING RESULT))
   (CL:LET* ((IN NULL))
    (CL:UNWIND-PROTECT
     (CL:PROGN (CL:SETQ IN (OPEN-URL-STREAM URL))
      (CL:SETQ RESULT (STREAM-TO-STRING IN)))
     (CL:WHEN (CL:NOT (CL:EQ IN NULL)) (CLOSE-STREAM IN))))
   RESULT))
...
CAUTION: String conversion will not do the right thing for binary data in Java.\" :PUBLIC? TRUE :COMMAND? TRUE)"
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFCONSTANT HTTP-DEFAULT-PORT INTEGER 80)" NULL)

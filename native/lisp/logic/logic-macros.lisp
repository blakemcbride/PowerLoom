...
(CL:DEFVAR SYM-LOGIC-MACROS-STELLA-FIRST-DEFINED NULL)
...
(CL:DEFVAR SYM-LOGIC-MACROS-LOGIC-*DATA-FINDER-KLUDGES-ENABLED?* NULL)
...
`*module*' and `*context*' and shield the outer bindings from changes."
  (LIST* SYM-LOGIC-MACROS-STELLA-SPECIAL
   (LIST*
    (LIST* SYM-LOGIC-MACROS-STELLA-*MODULE*
     (LIST* SYM-LOGIC-MACROS-STELLA-FIRST-DEFINED MODULEFORM
      (CONS SYM-LOGIC-MACROS-STELLA-*MODULE* NIL))
     NIL)
    (LIST* SYM-LOGIC-MACROS-STELLA-*CONTEXT*
     SYM-LOGIC-MACROS-STELLA-*MODULE* NIL)
    NIL)
   (CONCATENATE
    (CL:IF
     (CL:AND (SYMBOL? ENVIRONMENT)
      (CL:NOT (CL:EQ ENVIRONMENT SYM-LOGIC-MACROS-STELLA-NULL)))
     (CONS (LIST* SYM-LOGIC-MACROS-STELLA-IGNORE ENVIRONMENT NIL) NIL)
     NIL)
    (CONS
...
;;; (DEFGLOBAL *DATA-FINDER-KLUDGES-ENABLED?* ...)

(CL:DEFVAR *DATA-FINDER-KLUDGES-ENABLED?* CL:NIL)

;;; (DEFMACRO WHEN-DATA-FINDER-KLUDGES-ENABLED ...)

(CL:DEFUN WHEN-DATA-FINDER-KLUDGES-ENABLED (BODY)
  (LIST* SYM-LOGIC-MACROS-STELLA-WHEN
   SYM-LOGIC-MACROS-LOGIC-*DATA-FINDER-KLUDGES-ENABLED?*
   (CONCATENATE BODY NIL)))

(CL:DEFUN HELP-STARTUP-LOGIC-MACROS1 ()
  (CL:PROGN
   (CL:LET*
    ((STELLAMODULE *STELLA-MODULE*)
     (LOGICMODULE (GET-STELLA-MODULE "LOGIC" CL:T))
     (PLKERNELMODULE (GET-STELLA-MODULE "PL-KERNEL" CL:T)))
    (INTERN-SURROGATE-IN-MODULE "PROPOSITION.IF" STELLAMODULE CL:T)
    (INTERN-SURROGATE-IN-MODULE "=" STELLAMODULE CL:T)
    (IMPORT-SYMBOL (LOOKUP-SYMBOL-IN-MODULE "=" STELLAMODULE CL:T)
     PLKERNELMODULE)
    (IMPORT-SURROGATE
     (LOOKUP-SURROGATE-IN-MODULE "=" STELLAMODULE CL:T) PLKERNELMODULE)
    (CL:LET*
     ((NAME NULL)
      (ITER-000
       (GET-QUOTED-TREE "((\"LISTOF\" \"SETOF\") \"/LOGIC\")"
        "/LOGIC")))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET*
       ((SNAME NAME)
        (PLKERNELSYM
         (INTERN-SYMBOL-IN-MODULE (%WRAPPER-VALUE SNAME) PLKERNELMODULE
          CL:T))
        (PLKERNELSGT
         (INTERN-SURROGATE-IN-MODULE (%WRAPPER-VALUE SNAME)
          PLKERNELMODULE CL:T)))
       (IMPORT-SYMBOL PLKERNELSYM LOGICMODULE)
       (IMPORT-SURROGATE PLKERNELSGT LOGICMODULE))
      (CL:SETQ ITER-000 (%%REST ITER-000)))))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ SYM-LOGIC-MACROS-STELLA-INTEGER
     (INTERN-RIGID-SYMBOL-WRT-MODULE "INTEGER"
      (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ SYM-LOGIC-MACROS-STELLA-FIRST-DEFINED
     (INTERN-RIGID-SYMBOL-WRT-MODULE "FIRST-DEFINED"
      (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    ...
    (CL:SETQ SYM-LOGIC-MACROS-LOGIC-*DATA-FINDER-KLUDGES-ENABLED?*
     (INTERN-RIGID-SYMBOL-WRT-MODULE "*DATA-FINDER-KLUDGES-ENABLED?*"
      NULL 0))
...
`*module*' and `*context*' and shield the outer bindings from changes.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "WHEN-DATA-FINDER-KLUDGES-ENABLED"
     "(DEFUN WHEN-DATA-FINDER-KLUDGES-ENABLED (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)"
     (CL:FUNCTION WHEN-DATA-FINDER-KLUDGES-ENABLED) NULL)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFSPECIAL *SUPPRESSNONLOGICOBJECTWARNING?* BOOLEAN TRUE)" NULL)
    (REGISTER-NATIVE-NAME SYM-LOGIC-MACROS-LOGIC-DEF-DISPLAY-GLOBALS
     KWD-LOGIC-MACROS-COMMON-LISP KWD-LOGIC-MACROS-FUNCTION)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *CYC-KLUDGES-ENABLED?* BOOLEAN FALSE)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *DATA-FINDER-KLUDGES-ENABLED?* BOOLEAN FALSE)" NULL))))

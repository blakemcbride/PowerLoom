...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* TRUE-WRAPPER FALSE-WRAPPER
     ...
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN
       (CL:LET*
        ...
        (CL:COND
         ((CL:AND (CL:NOT (CL:EQ DECODED-ZONE NULL))
           (CL:NOT (NULL-WRAPPER? DECODED-ZONE)))
          (%WRAPPER-VALUE DECODED-ZONE))
         ((CL:NOT (CL:EQ DECODED-ZONE NULL)) (GET-LOCAL-TIME-ZONE))
         (CL:T NULL-FLOAT)))))
     (CL:T NULL-FLOAT)))))
...
(CL:DEFUN IS-CURRENT-TIME-SPEC? (SPEC)
  (CL:COND
   ((SUBTYPE-OF-STRING? (SAFE-PRIMARY-TYPE SPEC))
    (CL:PROGN (STRING-EQUAL? (%WRAPPER-VALUE SPEC) "now")))
   (CL:T CL:NIL)))
...
(CL:DEFUN FORMAT-TIMEPOINT-COMPUTATION (TIMEPOINT TIMEZONE CONTROL)
  ...
  (CL:WHEN (CL:NOT (NULL-WRAPPER? TIMEZONE))
   (CL:HANDLER-CASE
    (CL:RETURN-FROM FORMAT-TIMEPOINT-COMPUTATION
     (WRAP-STRING
      (FORMAT-DATE (%WRAPPER-VALUE TIMEPOINT) TIMEZONE
       (%WRAPPER-VALUE CONTROL))))
    (CL:CONDITION ())))
  NULL)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-METHOD-OBJECT
     ...
     (WRAP-METHOD-CODE (CL:FUNCTION GENERATE-SPECIALIZED-TERM)) NULL)
    (DEFINE-METHOD-OBJECT
...
that all logic-wrapped DATE-TIME-OBJECTs are the same object.\")"
    ...
    (DEFINE-FUNCTION-OBJECT "IS-CURRENT-TIME-SPEC?"
     "(DEFUN (IS-CURRENT-TIME-SPEC? BOOLEAN) ((SPEC OBJECT)))"
     (CL:FUNCTION IS-CURRENT-TIME-SPEC?) NULL)

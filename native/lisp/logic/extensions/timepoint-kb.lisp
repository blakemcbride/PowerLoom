   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ
     ...
     (INTERN-RIGID-SYMBOL-WRT-MODULE "TIMEPOINT-TIME-COMPUTATION" NULL
      0))
    (CL:SETQ SYM-TIMEPOINT-KB-TIMEPOINT-KB-FORMAT-TIMEPOINT
     (INTERN-RIGID-SYMBOL-WRT-MODULE "FORMAT-TIMEPOINT"
      (GET-STELLA-MODULE "/TIMEPOINT-KB" CL:T) 0))
    (CL:SETQ
     SYM-TIMEPOINT-KB-TIMEPOINT-SUPPORT-FORMAT-TIMEPOINT-COMPUTATION
     (INTERN-RIGID-SYMBOL-WRT-MODULE "FORMAT-TIMEPOINT-COMPUTATION"
      NULL 0))
    (CL:SETQ SYM-TIMEPOINT-KB-TIMEPOINT-KB-CURRENT-TIME
     (INTERN-RIGID-SYMBOL-WRT-MODULE "CURRENT-TIME"
      (GET-STELLA-MODULE "/TIMEPOINT-KB" CL:T) 0))
    (CL:SETQ KWD-TIMEPOINT-KB-<<=
     (INTERN-RIGID-SYMBOL-WRT-MODULE "<<=" NULL 2))
...
As a ?zone input value common strings are also recognized (see timepoint-of*)."))
     ...
     (%DEFFUNCTION
      (CONS-LIST
       (COPY-CONS-TREE SYM-TIMEPOINT-KB-TIMEPOINT-KB-FORMAT-TIMEPOINT)
       (COPY-CONS-TREE
        (GET-QUOTED-TREE
         "((?TP ?ZONE (?CONTROL STRING) (?FORMATTED STRING)) \"/TIMEPOINT-SUPPORT\")"
         "/TIMEPOINT-SUPPORT"))
       KWD-TIMEPOINT-KB-DOCUMENTATION
       (COPY-CONS-TREE
        (WRAP-STRING
         "Format ?tp relative to ?zone according to the format ?control string whose
...
As a ?zone input value common strings are also recognized (see timepoint-of*)."))
     ...
     (%DEFRELATION
      (CONS-LIST
       (COPY-CONS-TREE SYM-TIMEPOINT-KB-TIMEPOINT-KB-CURRENT-TIME)
       (COPY-CONS-TREE
        (GET-QUOTED-TREE "((?TP) \"/TIMEPOINT-SUPPORT\")"
         "/TIMEPOINT-SUPPORT"))
       KWD-TIMEPOINT-KB-DOCUMENTATION
       (COPY-CONS-TREE
        (WRAP-STRING
         "Convenience relation to access the current time ?tp.  Every time this is evaluated
...
into an assertion via `assert-from-query'.  For example:
    (assert-from-query (retrieve (current-time ?now)) :pattern (kappa (?now) (mytimeof t1 ?now)))"))
       ...
       (COPY-CONS-TREE
        (GET-QUOTED-TREE
         "((EXISTS ?SPEC (AND (MEMBER-OF ?SPEC (LISTOF \"now\")) (/TIMEPOINT-KB/TIMEPOINT-OF ?SPEC ?TP))) \"/TIMEPOINT-SUPPORT\")"
         "/TIMEPOINT-SUPPORT"))))
     (PROCESS-DEFINITIONS)))))

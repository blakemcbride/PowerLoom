...
circumstances such as inside specialist computations, etc."
  (GET-NAME-IN-MODULE (STELLA::%SKOLEM-NAME SKOLEM)
...
circumstances such as inside specialist computations, etc."
  (STELLA::%SYMBOL-NAME (STELLA::%SKOLEM-NAME SKOLEM)))
...
(CL:DEFUN GET-VARIABLE-NAME (VARIABLE)
  ...
  (GET-NAME-IN-MODULE (STELLA::%SKOLEM-NAME VARIABLE)
   STELLA::*ROOT-MODULE* STELLA::NULL))
...
(CL:DEFUN GET-VARIABLE-SHORT-NAME (VARIABLE)
  ...
  (STELLA::%SYMBOL-NAME (STELLA::%SKOLEM-NAME VARIABLE)))
...
(CL:DEFUN GET-STELLA-TYPE-NAME (SELF)
  ...
  (CL:LET* ((TYPE (STELLA::PRIMARY-TYPE SELF)))
   (STELLA::COMPUTE-FULL-NAME (STELLA::%SYMBOL-NAME TYPE)
    (STELLA::%HOME-CONTEXT TYPE))))
...
(CL:DEFUN GET-STELLA-TYPE-SHORT-NAME (SELF)
  ...
  (STELLA::%SYMBOL-NAME (STELLA::PRIMARY-TYPE SELF)))
...
(CL:DEFUN GET-LOGIC-TYPE-SHORT-NAME (SELF)
  ...
  (STELLA::%SYMBOL-NAME (STELLA::LOGICAL-TYPE SELF)))
...
name will be all upper case (This latter feature may change!)"
  ...
  (CL:LET*
   ...
   (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
    ...
    (CL:WHEN
     (CL:NOT
      (CL:OR (STELLA::%CASE-SENSITIVE? MODULE)
    ...
    ((STELLA::SUBTYPE-OF? TEST-VALUE-000 SGT-PLI-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:IF (STELLA::FUNCTION-OUTPUT-SKOLEM? OBJ)
       (GET-COLUMN-COUNT (STELLA::%DEFINING-PROPOSITION OBJ)) 1)))
    (CL:T 1))))
    ...
    ((STELLA::SUBTYPE-OF? TEST-VALUE-000 SGT-PLI-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:IF (STELLA::FUNCTION-OUTPUT-SKOLEM? SEQUENCE)
       (GET-NTH-VALUE (STELLA::%DEFINING-PROPOSITION SEQUENCE) N MODULE
        ENVIRONMENT)
       SEQUENCE)))
    (CL:T SEQUENCE))))
...
This will be the actual number of arguments, even if the proposition is of variable arity."
  (STELLA::LENGTH (STELLA::%ARGUMENTS (STELLA::%PROPOSITION FRAME))))
...
that is currently being inferred by `frame'."
  (STELLA::DYNAMIC-SLOT-VALUE (STELLA::%DYNAMIC-SLOTS FRAME)
   SYM-PLI-STELLA-ITERATOR STELLA::NULL))
...
that is currently being inferred by `frame' to `iterator'."
  (STELLA::SET-DYNAMIC-SLOT-VALUE (STELLA::%DYNAMIC-SLOTS FRAME)
   SYM-PLI-STELLA-ITERATOR ITERATOR STELLA::NULL))
...
by `frame'.  This will generally be a pattern variable that might be unbound or bound."
  ...
  #+MCL
  (CL:CHECK-TYPE N CL:FIXNUM)
  (CL:LET* ((ARGS (STELLA::%ARGUMENTS (STELLA::%PROPOSITION FRAME))))
   (CL:IF (CL:AND (CL:>= N 0) (CL:< N (STELLA::LENGTH ARGS)))
    (CL:AREF (CL:THE CL:SIMPLE-VECTOR (STELLA::%THE-ARRAY ARGS)) N)
    (CL:LET* ((STREAM-000 (STELLA::NEW-OUTPUT-STRING-STREAM)))
     (STELLA::%%PRINT-STREAM (STELLA::%NATIVE-STREAM STREAM-000)
    ...
    ((CL:<= (CL:THE CL:FIXNUM (CL:LENGTH PATTERN))
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE
       ...
       (CL:SETQ ARG
        (CL:AREF
         (CL:THE CL:SIMPLE-VECTOR (STELLA::%THE-ARRAY VECTOR-001))
         INDEX-001))
       (CL:CASE CH
        (#\B
         (CL:WHEN
...
The syntax of the pattern is the same as used for `is-frame-binding-pattern' (which see)."
  (CL:LET*
   ((ARGS (STELLA::%ARGUMENTS (STELLA::%PROPOSITION FRAME)))
...
all frame variables to the state they were in when the binding stack state was recorded."
  (CL:LET*
   ((PATTERNRECORD
     (STELLA::%CURRENT-PATTERN-RECORD STELLA::*QUERYITERATOR*))
    (UBSTACKOFFSET
     (STELLA::%TOP-UNBINDING-STACK-OFFSET PATTERNRECORD)))
...
;;; (DEFCLASS OBJECT-REGISTRY-LIST ...)

(CL:DEFCLASS OBJECT-REGISTRY-LIST (STELLA::LIST)
  ((REGISTRY-LENGTH :TYPE CL:FIXNUM :INITFORM STELLA::NULL-INTEGER
    :ALLOCATION :INSTANCE :ACCESSOR %REGISTRY-LENGTH)
   (N-FREE-CELLS :TYPE CL:FIXNUM :INITFORM STELLA::NULL-INTEGER
    :ALLOCATION :INSTANCE :ACCESSOR %N-FREE-CELLS)))

(CL:DEFUN NEW-OBJECT-REGISTRY-LIST ()
  (CL:LET* ((SELF STELLA::NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE OBJECT-REGISTRY-LIST)))
   (CL:SETF (STELLA::%THE-CONS-LIST SELF) STELLA::NIL)
   (CL:SETF (%N-FREE-CELLS SELF) 0) (CL:SETF (%REGISTRY-LENGTH SELF) 0)
   SELF))
...
(CL:DEFUN ACCESS-OBJECT-REGISTRY-LIST-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-PLI-PLI-REGISTRY-LENGTH)
    (CL:IF SETVALUE?
     (CL:SETF (%REGISTRY-LENGTH SELF) (STELLA::%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (STELLA::WRAP-INTEGER (%REGISTRY-LENGTH SELF)))))
   ((CL:EQ SLOTNAME SYM-PLI-PLI-N-FREE-CELLS)
    (CL:IF SETVALUE?
     (CL:SETF (%N-FREE-CELLS SELF) (STELLA::%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (STELLA::WRAP-INTEGER (%N-FREE-CELLS SELF)))))
   (CL:T
    (CL:LET* ((STREAM-000 (STELLA::NEW-OUTPUT-STRING-STREAM)))
     (STELLA::%%PRINT-STREAM (STELLA::%NATIVE-STREAM STREAM-000) "`"
      SLOTNAME "' is not a valid case option")
...
This is not thread safe and needs to be explicitly synchronized in a threaded environment."
  (CL:LET* ((REGISTRY *OBJECT-REGISTRY*))
   ...
   (CL:SETF (%REGISTRY-LENGTH REGISTRY)
    (CL:1+ (%REGISTRY-LENGTH REGISTRY)))
   (STELLA::%THE-CONS-LIST REGISTRY)))
...
in a threaded environment."
  (CL:LET*
   ((REGISTRY *OBJECT-REGISTRY*) (NFREE (%N-FREE-CELLS REGISTRY)))
   ...
   (CL:WHEN
    (CL:AND (CL:> NFREE 1000)
     (CL:>= NFREE
      (CL:LET ((ARG (%REGISTRY-LENGTH REGISTRY)) (COUNT 1))
       (CL:DECLARE (CL:TYPE CL:FIXNUM ARG COUNT))
       (CL:THE CL:FIXNUM
        (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))))
    (STELLA::REMOVE REGISTRY STELLA::NULL)
    (CL:SETF (%REGISTRY-LENGTH REGISTRY)
     (CL:- (%REGISTRY-LENGTH REGISTRY) NFREE))
    (CL:SETQ NFREE 0))
   (CL:SETF (%N-FREE-CELLS REGISTRY) NFREE)))

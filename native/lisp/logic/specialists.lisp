...
(CL:DEFVAR SGT-SPECIALISTS-PL-KERNEL-KB-HOLDS NULL)
...
(CL:DEFVAR SGT-SPECIALISTS-PL-KERNEL-KB-COUNT-MEMBERS NULL)
...
(CL:DEFVAR KWD-SPECIALISTS-JUST-IN-TIME NULL)
...
(CL:DECLAIM
  ...
  *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE *INFERENCELEVEL*
  SHALLOW-INFERENCE SUBSUMPTION-INFERENCE BACKTRACKING-INFERENCE
  NORMAL-INFERENCE *TRACED-KEYWORDS* NIL-LIST *RECORD-JUSTIFICATIONS?*
  *CONTEXT* EMPTY-PROPOSITIONS-ITERATOR REFUTATION-INFERENCE
  ASSERTION-INFERENCE FALSE-WRAPPER NIL NULL-INTEGER TRUE-WRAPPER
  ...
  *QUERYITERATOR* TRUE-TRUTH-VALUE *REVERSEPOLARITY?* *MODULE*
...
(CL:DEFUN STELLA-FUNCTION-FROM-PROCEDURE (PROCEDURE)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ STELLAFUNCTION NULL)
    (CL:LET*
     ((*MODULE* (HOME-MODULE (%SURROGATE-VALUE-INVERSE PROCEDURE)))
      (*CONTEXT* *MODULE*))
     (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (CL:SETQ STELLAFUNCTION
      (LOOKUP-FUNCTION-BY-NAME (%SYMBOL-NAME PROCEDURENAME)))))
   (CL:WHEN (CL:EQ STELLAFUNCTION NULL)
    (CL:SETQ STELLAFUNCTION
     (LOOKUP-FUNCTION-BY-NAME (%SYMBOL-NAME PROCEDURENAME))))
   (CL:WHEN (CL:EQ STELLAFUNCTION NULL)
...
;;; (DEFUN (SUCCESS-RESULT? BOOLEAN) ...)

(CL:DEFUN SUCCESS-RESULT? (RESULT)
  (CL:OR (CL:EQ RESULT KWD-SPECIALISTS-CONTINUING-SUCCESS)
   (CL:EQ RESULT KWD-SPECIALISTS-FINAL-SUCCESS)))

;;; (DEFUN (FAILURE-RESULT? BOOLEAN) ...)

(CL:DEFUN FAILURE-RESULT? (RESULT)
  (CL:OR (CL:EQ RESULT KWD-SPECIALISTS-FAILURE)
   (CL:EQ RESULT KWD-SPECIALISTS-TERMINAL-FAILURE)))

;;; (DEFUN (CONTINUING-RESULT? BOOLEAN) ...)

(CL:DEFUN CONTINUING-RESULT? (RESULT)
  (CL:OR (CL:EQ RESULT KWD-SPECIALISTS-CONTINUING-SUCCESS)
   (CL:EQ RESULT KWD-SPECIALISTS-FAILURE)))

;;; (DEFUN (FINAL-RESULT? BOOLEAN) ...)

(CL:DEFUN FINAL-RESULT? (RESULT)
  (CL:OR (CL:EQ RESULT KWD-SPECIALISTS-FINAL-SUCCESS)
   (CL:EQ RESULT KWD-SPECIALISTS-TERMINAL-FAILURE)))
               ...
               (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
                (CL:SETQ VAR
                 (CL:AREF
                  ...
                  INDEX-000))
                (CL:WHEN (CL:EQ (SAFE-ARGUMENT-BOUND-TO VAR) NULL)
...
;;; (DEFUN (/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST KEYWORD) ...)

(CL:DEFUN |/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST| (FRAME LASTMOVE)
  (CL:LET*
   ((PROPOSITION (%PROPOSITION FRAME))
    (ARGUMENTS (%ARGUMENTS PROPOSITION))
    (MEMBERARG
   ...
   (CL:COND
    ((CL:EQ LASTMOVE KWD-SPECIALISTS-UP-TRUE)
     (CL:RETURN-FROM
      |/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST|
      (CL:IF (CL:NOT (CL:EQ (%DOWN FRAME) NULL))
       KWD-SPECIALISTS-CONTINUING-SUCCESS
       KWD-SPECIALISTS-FINAL-SUCCESS)))
    ((CL:EQ LASTMOVE KWD-SPECIALISTS-UP-FAIL)
       ...
       (CL:T
        (CL:WHEN (BIND-VARIABLE-TO-VALUE? COLLECTIONARG VALUE CL:T)
         (CL:RETURN-FROM
          |/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST|
          KWD-SPECIALISTS-CONTINUING-SUCCESS))))))
    (CL:RETURN-FROM |/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST|
     KWD-SPECIALISTS-FAILURE))
   KWD-SPECIALISTS-FAILURE))
  ...
  (CL:LET*
   ((ARGUMENTS (%ARGUMENTS (%PROPOSITION FRAME)))
    ...
    (COLLECTIONARG
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS)) 1)))
   (CL:WHEN
    (CL:AND (CL:EQ LASTMOVE KWD-SPECIALISTS-DOWN)
    ...
    (CL:LET*
     ((MEMBER (ARGUMENT-BOUND-TO MEMBERARG))
      (COLLECTION (ARGUMENT-BOUND-TO COLLECTIONARG)))
     (CL:COND
      ((CL:AND (CL:NOT (CL:EQ MEMBER NULL))
        (CL:NOT (CL:EQ COLLECTION NULL)))
       (CL:IF (MEMBER? (ALL-DIRECT-TYPES MEMBER) COLLECTION)
      ...
      ((CL:NOT (CL:EQ MEMBER NULL))
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
        SYM-SPECIALISTS-STELLA-ITERATOR
        (ALLOCATE-ITERATOR (ALL-DIRECT-TYPES MEMBER)) NULL)))))
   (CL:LET*
    ((INSTANCEOFVALUE
      (|/PL-KERNEL-KB/INSTANCE-OF-SPECIALIST| FRAME LASTMOVE)))
    (CL:COND
     ((CL:OR (CL:EQ INSTANCEOFVALUE KWD-SPECIALISTS-CONTINUING-SUCCESS)
       (CL:EQ INSTANCEOFVALUE KWD-SPECIALISTS-FINAL-SUCCESS))
      (CL:COND
       ((CL:NOT
         (CL:EQ
          (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
          ...
          NULL))
        INSTANCEOFVALUE)
       ((MEMBER? (ALL-DIRECT-TYPES (ARGUMENT-BOUND-TO MEMBERARG))
         (ARGUMENT-BOUND-TO COLLECTIONARG))
        INSTANCEOFVALUE)
       (CL:T KWD-SPECIALISTS-MOVE-DOWN)))
     (CL:T INSTANCEOFVALUE)))))

;;; (DEFUN (/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST KEYWORD) ...)

(CL:DEFUN |/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST| (FRAME LASTMOVE)
  (CL:SETQ LASTMOVE LASTMOVE)
  (CL:LET*
   ((PROPOSITION (%PROPOSITION FRAME))
    (ARGUMENTS (%ARGUMENTS PROPOSITION))
   ...
   (CL:WHEN (CL:EQ ITERATOR NULL)
    (CL:COND
     ((CL:NOT (CL:EQ MEMBER NULL))
      (CL:SETQ ITERATOR (ALL-TRUE-DEPENDENT-ISA-PROPOSITIONS MEMBER)))
     ((CL:AND (CL:NOT (CL:EQ COLLECTION NULL)) (CLASS? COLLECTION)
       (ISA? COLLECTION SGT-SPECIALISTS-LOGIC-NAMED-DESCRIPTION))
      (CL:SETQ ITERATOR
       (ALL-TRUE-EXTENSION-MEMBERS COLLECTION CL:NIL)))
     (CL:T
      (CL:RETURN-FROM |/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST|
       KWD-SPECIALISTS-TERMINAL-FAILURE)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
     SYM-SPECIALISTS-STELLA-ITERATOR ITERATOR NULL))
   (CL:LOOP WHILE (NEXT? ITERATOR) DO
    (CL:LET*
     ((ISAPROP (%VALUE ITERATOR))
      (ISATYPE (GET-DESCRIPTION (%OPERATOR ISAPROP)))
      (ISAARG
       (CL:AREF
        (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS ISAPROP)))
        0)))
     (CL:WHEN
      (CL:AND (ASSERTED-TRUE? ISAPROP)
       (BIND-ARGUMENT-TO-VALUE? COLLECTIONARG ISATYPE CL:T)
       (BIND-ARGUMENT-TO-VALUE? MEMBERARG ISAARG CL:T))
      (CL:IF (CL:OR (CL:EQ MEMBER NULL) (CL:EQ COLLECTION NULL))
       (CL:RETURN-FROM |/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST|
        KWD-SPECIALISTS-CONTINUING-SUCCESS)
       (CL:RETURN-FROM |/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST|
        KWD-SPECIALISTS-FINAL-SUCCESS)))))
...
;;; (DEFUN (/PL-KERNEL-KB/THAT-COMPUTATION OBJECT) ...)

(CL:DEFUN |/PL-KERNEL-KB/THAT-COMPUTATION| (P)
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE P)
     SGT-SPECIALISTS-LOGIC-PROPOSITION)
    (CL:PROGN
     (CL:WHEN (ALL-ARGUMENTS-BOUND? P)
      (CL:LET* ((MAPPING (NEW-KEY-VALUE-MAP)) (THAT NULL))
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
         (CL:SETQ ARG
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
           INDEX-000))
         (CL:LET* ((VALUE (SAFE-ARGUMENT-BOUND-TO ARG)))
          (CL:WHEN (CL:NOT (EQL? ARG VALUE))
           (INSERT-AT MAPPING ARG VALUE)))
         (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
       (CL:SETQ THAT
        (FIND-MATCHING-NON-DESCRIPTIVE-PROPOSITION P MAPPING))
       (CL:WHEN
        (CL:AND (CL:NOT (CL:EQ THAT NULL))
         (VISIBLE-FROM? (HOME-MODULE THAT) *MODULE*))
        (CL:RETURN-FROM |/PL-KERNEL-KB/THAT-COMPUTATION| THAT))))))
   (CL:T))
  NULL)
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE P)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SPECIALISTS-LOGIC-PROPOSITION)
     (CL:PROGN
      (CL:LET* ((ARGUMENTS (%ARGUMENTS P)) (ARITY (LENGTH ARGUMENTS)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM ARITY))
       (CL:WHEN (CL:< (%WRAPPER-VALUE I) 0)
        (CL:SETQ I (WRAP-INTEGER (CL:+ ARITY (%WRAPPER-VALUE I)))))
       (CL:WHEN
        (CL:AND (CL:>= (%WRAPPER-VALUE I) 0)
         (CL:< (%WRAPPER-VALUE I) ARITY))
        (CL:RETURN-FROM
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SPECIALISTS-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? P)
       (CL:LET*
        ((PROP (%DEFINING-PROPOSITION P)) (ARGUMENTS (%ARGUMENTS PROP))
         (ARITY (LENGTH ARGUMENTS)))
        (CL:DECLARE (CL:TYPE CL:FIXNUM ARITY))
        (CL:WHEN (CL:< (%WRAPPER-VALUE I) 0)
         (CL:SETQ I
          (WRAP-INTEGER (CL:1- (CL:+ ARITY (%WRAPPER-VALUE I))))))
        (CL:WHEN
         (CL:AND (CL:>= (%WRAPPER-VALUE I) 0)
          (CL:< (%WRAPPER-VALUE I) (CL:1- ARITY)))
         (CL:RETURN-FROM
          |/PL-KERNEL-KB/PROPOSITION-ARGUMENT-COMPUTATION|
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS))
           (CL:THE CL:FIXNUM (%WRAPPER-VALUE I)))))))))
...
;;; (DEFUN (/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST KEYWORD) ...)

(CL:DEFUN |/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST| (FRAME LASTMOVE)
  (CL:SETQ LASTMOVE LASTMOVE)
  (CL:LET*
   ((ARGUMENTS (%ARGUMENTS (%PROPOSITION FRAME)))
    (GOALVALUE
    ...
    (ITERATOR
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
      SYM-SPECIALISTS-STELLA-ITERATOR NULL)))
   (CL:WHEN
    (CL:OR (CL:EQ GOALVALUE NULL)
     (CL:NOT (ISA? GOALVALUE SGT-SPECIALISTS-LOGIC-PROPOSITION)))
    (CL:RETURN-FROM
     |/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST|
     KWD-SPECIALISTS-TERMINAL-FAILURE))
   (CL:SETQ GOALPROP GOALVALUE)
   (CL:SETQ HOLDS?
    (CL:EQ (%OPERATOR GOALPROP) SGT-SPECIALISTS-PL-KERNEL-KB-HOLDS))
   (CL:WHEN (CL:EQ ITERATOR NULL)
    (CL:COND
     (HOLDS?
      (CL:LET*
       ((GOALARGUMENTS (%ARGUMENTS GOALPROP))
        (RELATION
         (ARGUMENT-BOUND-TO
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY GOALARGUMENTS))
           0)))
        (BACKLINKEDARG NULL))
       (CL:COND
        ((CL:NOT (CL:EQ RELATION NULL))
         (CL:SETQ RELATION (OBJECT-SURROGATE RELATION))
         (CL:LET* ((VALUE-000 NIL))
          (CL:LET*
           ((I NULL-INTEGER) (ITER-000 1)
            (UPPER-BOUND-000 (CL:1- (LENGTH GOALARGUMENTS)))
            (COLLECT-000 NULL))
           (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
           (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
            (CL:SETQ I ITER-000)
            (CL:IF (CL:EQ COLLECT-000 NULL)
             (CL:PROGN
              (CL:SETQ COLLECT-000
               (CONS
                (ARGUMENT-BOUND-TO
                 (CL:AREF
                  (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY GOALARGUMENTS))
                  I))
                NIL))
              (CL:IF (CL:EQ VALUE-000 NIL)
               (CL:SETQ VALUE-000 COLLECT-000)
             ...
             (CL:PROGN
              ...
              (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
            (CL:SETQ ITER-000 (CL:1+ ITER-000))))
          (CL:SETQ BACKLINKEDARG
           (SELECT-INSTANCE-WITH-BACKLINKS VALUE-000 RELATION))))
        (CL:T
         (CL:SETQ BACKLINKEDARG
          (SELECT-ARGUMENT-WITH-BACKLINKS GOALPROP))))
       (CL:COND
        ((CL:NOT (CL:EQ BACKLINKEDARG NULL))
         (CL:SETQ ITERATOR
          (ALLOCATE-ITERATOR
           (UNFILTERED-DEPENDENT-PROPOSITIONS BACKLINKEDARG
            RELATION))))
        ((CL:NOT (CL:EQ RELATION NULL))
         (CL:SETQ ITERATOR
          (ALL-EXTENSION-MEMBERS
           (SURROGATE-TO-DESCRIPTION RELATION)))))))
     (CL:T
      (CL:SETQ ITERATOR
       (TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR
        (ALL-MATCHING-PROPOSITIONS GOALPROP)))))
   ...
   (CL:WHEN (CL:NOT (CL:EQ ITERATOR NULL))
    (CL:LOOP WHILE (NEXT? ITERATOR) DO
     (CL:WHEN
      (CL:AND (VISIBLE-FROM? (HOME-MODULE (%VALUE ITERATOR)) *CONTEXT*)
       (ALL-ARGUMENTS-BOUND? (%VALUE ITERATOR))
       (RELATION-AND-ARGUMENTS-UNIFY? (%VALUE ITERATOR) GOALPROP
        CL:NIL)
       (BIND-ARGUMENT-TO-VALUE? TERMARG (%VALUE ITERATOR) CL:T))
      (CL:RETURN-FROM
       |/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST|
       KWD-SPECIALISTS-CONTINUING-SUCCESS))))
   KWD-SPECIALISTS-TERMINAL-FAILURE))
       ...
       (CL:WHEN SUCCESS?
        (CL:LET* ((SOLUTION (POP (%SOLUTIONS SUBQUERYITERATOR))))
         ...
         (CL:WHEN PARTIALQUERY?
          (CL:SETF (%MATCH-SCORE (%PARTIAL-MATCH-FRAME RESULT))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000
      SGT-SPECIALISTS-LOGIC-NAMED-DESCRIPTION)
     (CL:PROGN
      (CL:OR (TEST-PROPERTY? SELF SGT-SPECIALISTS-PL-KERNEL-KB-CLOSED)
       (CL:AND
        (CL:NOT
         (CL:EQ
          (ACCESS-IN-CONTEXT
   ...
   ((PROPOSITION (%PROPOSITION FRAME))
    ...
    (LISTARG
     (CL:AREF
      (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS PROPOSITION)))
      1))
    (LIST? CL:NIL) (UNIQUE? CL:T) (COUNT? CL:NIL) (MEMBERS NULL)
    (RESULT NULL))
   (CL:SETQ LASTMOVE LASTMOVE)
   ...
   (CL:LET* ((TEST-VALUE-000 (%OPERATOR PROPOSITION)))
    (CL:COND
     ...
     ((CL:EQ TEST-VALUE-000
      ...
      (CL:SETQ LIST? CL:T))
     ((CL:EQ TEST-VALUE-000 SGT-SPECIALISTS-PL-KERNEL-KB-COUNT-MEMBERS)
      (CL:SETQ COUNT? CL:T))
     (CL:T)))
   (CL:COND
    ((CL:AND
             ...
             (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
              (CL:SETQ V
               (CL:AREF
                (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
                INDEX-000))
              (CL:WHEN (CL:EQ (SAFE-ARGUMENT-BOUND-TO V) NULL)
               (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
              (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
     ...
     ((CL:NOT (CL:EQ RESULT NULL))
      (CL:WHEN COUNT?
       (CL:SETQ RESULT
        (WRAP-INTEGER
         (CL:1-
          (LENGTH (%ARGUMENTS (%DEFINING-PROPOSITION RESULT))))))))
     (CL:T
      (CL:WHEN UNIQUE?
       (CL:SETQ MEMBERS
        (CL:IF (CONS? (FIRST MEMBERS))
         (REMOVE-DUPLICATES-EQUAL MEMBERS)
         (REMOVE-DUPLICATES MEMBERS))))
      (CL:COND
       (COUNT? (CL:SETQ RESULT (WRAP-INTEGER (LENGTH MEMBERS))))
       (LIST? (CL:SETQ RESULT (CREATE-LOGICAL-LIST MEMBERS)))
       (CL:T (CL:SETQ RESULT (CREATE-ENUMERATED-SET MEMBERS)))))))
   (CL:LET*
    ((SUCCESS
      ...
      ((CL:AND
        (CL:EQ
         (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
          SYM-SPECIALISTS-STELLA-ITERATOR NULL)
         NULL)
        (CL:EQ N NULL) (CL:NOT (CL:EQ ELEMENT NULL))
        (ENUMERATED-SET? COLLECTION))
       (CL:LET*
        ((ARG NULL) (VECTOR-000 ARGUMENTS) (INDEX-000 0)
         (LENGTH-000 (LENGTH VECTOR-000)) (I NULL-INTEGER) (ITER-000 0)
      ...
      (CL:T
       (CL:LET*
        ...
        (CL:WHEN (CL:EQ ITERATOR NULL)
         ...
         (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
          SYM-SPECIALISTS-STELLA-ITERATOR ITERATOR NULL))
        (CL:LET* ((ELEMENT NULL) (ITER-001 ITERATOR))
         (CL:LOOP WHILE (NEXT? ITER-001) DO
          (CL:SETQ ELEMENT (%VALUE ITER-001))
          (CL:WHEN (CL:<= (%ITERATOR-INTEGER ITERATOR) NARGS)
           (CL:SETQ THEN (CL:1- (%ITERATOR-INTEGER ITERATOR)))
           (CL:IF
            (CL:AND
...
;;; (DEFUN (/PL-KERNEL-KB/OBJECT-CONTEXT-COMPUTATION CONTEXT) ...)

(CL:DEFUN |/PL-KERNEL-KB/OBJECT-CONTEXT-COMPUTATION| (INSTANCE)
  (HOME-MODULE INSTANCE))

;;; (DEFUN (/PL-KERNEL-KB/OBJECT-NAME-CONTEXT-COMPUTATION CONTEXT) ...)

(CL:DEFUN |/PL-KERNEL-KB/OBJECT-NAME-CONTEXT-COMPUTATION| (INSTANCE)
  (CL:LET* ((NAMESYMBOL (OBJECT-SURROGATE (VALUE-OF INSTANCE))))
   (CL:IF (CL:EQ NAMESYMBOL NULL) NULL (HOME-MODULE NAMESYMBOL))))
     ...
     (CL:WHEN (CL:EQ ADJUNCT NULL)
      (CL:LET*
       ...
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
        SYM-SPECIALISTS-LOGIC-PROOF-ADJUNCT ADJUNCT NULL)))
     (CL:SETF (%DOWN FRAME) (%DOWN-FRAME ADJUNCT))
     (GET-INFERENCE-CACHE CONTEXTVALUE KWD-SPECIALISTS-JUST-IN-TIME)
...
;;; (DEFUN (/PL-KERNEL-KB/EQUIVALENT-TERMS-COMPUTATION OBJECT) ...)

(CL:DEFUN |/PL-KERNEL-KB/EQUIVALENT-TERMS-COMPUTATION| (TERM)
  (CL:LET* ((EQUIVS (CONS TERM (GET-ALL-EQUIVALENT-VALUES TERM))))
   (CL:SETQ EQUIVS (SORT EQUIVS (CL:FUNCTION LOGIC-FORM-LESS?)))
   (CREATE-LOGICAL-LIST (LISTIFY EQUIVS))))
...
(CL:DEFUN HELP-STARTUP-SPECIALISTS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-SPECIALISTS-PL-KERNEL-KB-HOLDS
  ...
  (CL:PROGN
   ...
   (CL:SETQ SGT-SPECIALISTS-PL-KERNEL-KB-COLLECT-INTO-ORDERED-SET
    (INTERN-RIGID-SYMBOL-WRT-MODULE "COLLECT-INTO-ORDERED-SET"
     (GET-STELLA-MODULE "/PL-KERNEL-KB" CL:T) 1))
   (CL:SETQ SGT-SPECIALISTS-PL-KERNEL-KB-COUNT-MEMBERS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "COUNT-MEMBERS"
     (GET-STELLA-MODULE "/PL-KERNEL-KB" CL:T) 1))
   ...
   (CL:SETQ SGT-SPECIALISTS-STELLA-CONTEXT
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CONTEXT"
     (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ KWD-SPECIALISTS-JUST-IN-TIME
...
(CL:DEFUN HELP-STARTUP-SPECIALISTS4 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "SUCCESS-RESULT?"
    "(DEFUN (SUCCESS-RESULT? BOOLEAN) ((RESULT KEYWORD)) :GLOBALLY-INLINE? TRUE (RETURN (OR (EQL? RESULT :CONTINUING-SUCCESS) (EQL? RESULT :FINAL-SUCCESS))))"
    (CL:FUNCTION SUCCESS-RESULT?) NULL)
   (DEFINE-FUNCTION-OBJECT "FAILURE-RESULT?"
    "(DEFUN (FAILURE-RESULT? BOOLEAN) ((RESULT KEYWORD)) :GLOBALLY-INLINE? TRUE (RETURN (OR (EQL? RESULT :FAILURE) (EQL? RESULT :TERMINAL-FAILURE))))"
    (CL:FUNCTION FAILURE-RESULT?) NULL)
   (DEFINE-FUNCTION-OBJECT "CONTINUING-RESULT?"
    "(DEFUN (CONTINUING-RESULT? BOOLEAN) ((RESULT KEYWORD)) :GLOBALLY-INLINE? TRUE (RETURN (OR (EQL? RESULT :CONTINUING-SUCCESS) (EQL? RESULT :FAILURE))))"
    (CL:FUNCTION CONTINUING-RESULT?) NULL)
   (DEFINE-FUNCTION-OBJECT "FINAL-RESULT?"
    "(DEFUN (FINAL-RESULT? BOOLEAN) ((RESULT KEYWORD)) :GLOBALLY-INLINE? TRUE (RETURN (OR (EQL? RESULT :FINAL-SUCCESS) (EQL? RESULT :TERMINAL-FAILURE))))"
    (CL:FUNCTION FINAL-RESULT?) NULL)
...
keyword indicating success or failure of a proof.\")"
   ...
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST"
    "(DEFUN (/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST KEYWORD) ((FRAME CONTROL-FRAME) (LASTMOVE KEYWORD)))"
    (CL:FUNCTION |/PL-KERNEL-KB/BUGGY-DIRECT-INSTANCE-OF-SPECIALIST|)
    NULL)
   (DEFINE-FUNCTION-OBJECT
   ...
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST"
    "(DEFUN (/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST KEYWORD) ((FRAME CONTROL-FRAME) (LASTMOVE KEYWORD)))"
    (CL:FUNCTION |/PL-KERNEL-KB/ASSERTED-INSTANCE-OF-SPECIALIST|) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT " /PL-KERNEL-KB/THAT-COMPUTATION"
    "(DEFUN (/PL-KERNEL-KB/THAT-COMPUTATION OBJECT) ((P OBJECT)))"
    (CL:FUNCTION |/PL-KERNEL-KB/THAT-COMPUTATION|) NULL)
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/PROPOSITION-RELATION-COMPUTATION"
   ...
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST"
    "(DEFUN (/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST KEYWORD) ((FRAME CONTROL-FRAME) (LASTMOVE KEYWORD)))"
    (CL:FUNCTION |/PL-KERNEL-KB/CONCEIVED-PROPOSITION-TERM-SPECIALIST|)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CLOSED-BY-ASSERTION?"
    "(DEFUN (CLOSED-BY-ASSERTION? BOOLEAN) ((SELF OBJECT)) :GLOBALLY-INLINE? TRUE (RETURN (AND (TEST-PROPERTY? SELF /PL-KERNEL-KB/@CLOSED))))"
...
(CL:DEFUN HELP-STARTUP-SPECIALISTS5 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT " /PL-KERNEL-KB/OBJECT-CONTEXT-COMPUTATION"
    "(DEFUN (/PL-KERNEL-KB/OBJECT-CONTEXT-COMPUTATION CONTEXT) ((INSTANCE OBJECT)))"
    (CL:FUNCTION |/PL-KERNEL-KB/OBJECT-CONTEXT-COMPUTATION|) NULL)
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/OBJECT-NAME-CONTEXT-COMPUTATION"
    "(DEFUN (/PL-KERNEL-KB/OBJECT-NAME-CONTEXT-COMPUTATION CONTEXT) ((INSTANCE OBJECT)))"
    (CL:FUNCTION |/PL-KERNEL-KB/OBJECT-NAME-CONTEXT-COMPUTATION|) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT
    " /PL-KERNEL-KB/EQUIVALENT-TERMS-COMPUTATION"
    "(DEFUN (/PL-KERNEL-KB/EQUIVALENT-TERMS-COMPUTATION OBJECT) ((TERM OBJECT)))"
    (CL:FUNCTION |/PL-KERNEL-KB/EQUIVALENT-TERMS-COMPUTATION|) NULL)))
...
the resulting code or knowledge bases non-portable to other target languages.\")"
     ...
     (CL:FUNCTION
      REGISTER-COMPUTATION-FUNCTION-NAME-EVALUATOR-WRAPPER))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *PROTOTYPE-ID-COUNTER* INTEGER 0)" NULL)

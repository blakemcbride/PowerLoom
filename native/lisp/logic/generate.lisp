...
(CL:DEFVAR SYM-GENERATE-PL-KERNEL-KB-SETOF NULL)
(CL:DEFVAR SYM-GENERATE-PL-KERNEL-KB-LISTOF NULL)
    ...
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ M (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN
       ...
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CONS
    (CL:IF (NO-DUPLICATES? SELF) SYM-GENERATE-PL-KERNEL-KB-SETOF
     SYM-GENERATE-PL-KERNEL-KB-LISTOF)
...
(CL:DEFUN HELP-STARTUP-GENERATE1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-GENERATE-PL-KERNEL-KB-SETOF
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SETOF"
     (GET-STELLA-MODULE "/PL-KERNEL-KB" CL:T) 0))
   (CL:SETQ SYM-GENERATE-PL-KERNEL-KB-LISTOF
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "GENERATE-EXPRESSION"
     "(DEFUN (GENERATE-EXPRESSION OBJECT) ((SELF OBJECT) (CANONICALIZEVARIABLENAMES? BOOLEAN)) :DOCUMENTATION \"Return an s-expression representing the source expression for 'self'.\" :PUBLIC? TRUE)"

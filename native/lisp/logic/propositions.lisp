...
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION NULL)
...
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION-SETTER NULL)
...
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-MATCH-SCORE NULL)
...
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-PROPOSITION-NEURAL-NETWORK NULL)
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-NEURAL-NETWORK NULL)
(CL:DEFVAR SYM-PROPOSITIONS-LOGIC-NEURAL-NET NULL)
...
(CL:DEFVAR KWD-PROPOSITIONS-META NULL)
(CL:DEFVAR KWD-PROPOSITIONS-JUST-IN-TIME NULL)
...
(CL:DEFVAR KWD-PROPOSITIONS-SCORE NULL)
...
(CL:DECLAIM
  ...
  *REDEFINE-RELATION-HOOKS* *INHIBITOBJECTFINALIZATION?*
  *DESCRIPTIONUNDERCONSTRUCTION* *LOGICVARIABLETABLE*
  *INTEGER-MSB-MASK* STANDARD-ERROR
  *BACKLINK-ALL-PROPOSITION-ARGUMENTS?* *REVERSEPOLARITY?*
  *TERMUNDERCONSTRUCTION* *RECORD-JUSTIFICATIONS?*
  *DONTUSEDEFAULTKNOWLEDGE?* *QUERYITERATOR*
...
(CL:DEFCLASS LOGIC-OBJECT (CONTEXT-SENSITIVE-OBJECT)
  ((VARIABLE-VALUE-INVERSE :DOCUMENTATION
    "List of direct parents whose `variable-value' slot points to this object."
    :ALLOCATION :INSTANCE :ACCESSOR %VARIABLE-VALUE-INVERSE)
...
(CL:DEFUN NEW-TRUTH-VALUE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%VARIABLE-VALUE-INVERSE SELF) NULL)
   (CL:SETF (%HOME-CONTEXT SELF) *MODULE*)
   (CL:SETF (%MATCH-SCORE SELF) 0.0d0) (CL:SETF (%STRENGTH SELF) NULL)
...
(CL:DEFUN ACCESS-TRUTH-VALUE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-PROPOSITIONS-LOGIC-MATCH-SCORE)
    (CL:IF SETVALUE?
...
(CL:DEFUN ACCESS-PROPOSITION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   (CL:T
    (CL:COND
     ((CL:EQ SLOTNAME
       SYM-PROPOSITIONS-LOGIC-PROPOSITION-NEURAL-NETWORK)
      (CL:SETQ SLOTNAME SYM-PROPOSITIONS-LOGIC-NEURAL-NET))
     ((CL:EQ SLOTNAME SYM-PROPOSITIONS-LOGIC-NEURAL-NETWORK)
      (CL:SETQ SLOTNAME SYM-PROPOSITIONS-LOGIC-NEURAL-NET))
     (CL:T))
    (CL:IF SETVALUE?
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SLOTNAME VALUE NULL)
   ...
   (CL:COND
    ...
    ((CL:OR (CL:EQ TV TRUE-TRUTH-VALUE) (CL:EQ TV FALSE-TRUTH-VALUE)
     ...
     (CL:LET* ((SELF-000 (NEW-TRUTH-VALUE)))
      (CL:SETF (%MATCH-SCORE SELF-000) WEIGHT)
      (CL:LET*
...
;;; (DEFUN (ASSERTED-TRUE? BOOLEAN) ...)

(CL:DEFUN ASSERTED-TRUE? (SELF)
  "Return true if `self' has been asserted to be true."
  (CL:LET*
   ((*CONTEXT* (GET-WORLD-STATE *CONTEXT*))
    (*MODULE* (%BASE-MODULE *CONTEXT*)))
   (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*)) (TRUE? SELF)))

;;; (DEFUN (ASSERTED-FALSE? BOOLEAN) ...)

(CL:DEFUN ASSERTED-FALSE? (SELF)
  "Return true if `self' has been asserted to be false."
  (CL:LET*
   ((*CONTEXT* (GET-WORLD-STATE *CONTEXT*))
    (*MODULE* (%BASE-MODULE *CONTEXT*)))
   (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*)) (FALSE? SELF)))
   ...
   (CL:WHEN *RECORD-JUSTIFICATIONS?*
    (CL:LET*
     ((NEGATED-PROPOSITION (CONCEIVE-INVERTED-PROPOSITION PROPOSITION))
      (ANTECEDENTS NIL))
     (CL:IF (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? NEGATED-PROPOSITION)
      (CL:SETQ ANTECEDENTS
       (%THE-CONS-LIST
        (GET-SUPPORTED-FORWARD-JUSTIFICATIONS NEGATED-PROPOSITION)))
     ...
     (CL:IF (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? PROPOSITION)
      (CL:SETQ ANTECEDENTS
       (APPEND
        (%THE-CONS-LIST
         (GET-SUPPORTED-FORWARD-JUSTIFICATIONS PROPOSITION))
        ANTECEDENTS))
     ...
     (ADD-FORWARD-JUSTIFICATION PROPOSITION
      (CREATE-CLASH-JUSTIFICATION PROPOSITION ANTECEDENTS
       KWD-PROPOSITIONS-FORWARD))
     (ADD-FORWARD-JUSTIFICATION NEGATED-PROPOSITION
...
;;; (DEFUN VERIFY-SKOLEM-AND-VALUE-TYPES ...)

(CL:DEFUN VERIFY-SKOLEM-AND-VALUE-TYPES (FROMSKOLEM TOVALUE)
  (CL:LET*
...
;;; (DEFUN (SKOLEM-WITH-VARIABLE-VALUE-CYCLE? BOOLEAN) ...)

(CL:DEFUN SKOLEM-WITH-VARIABLE-VALUE-CYCLE? (SELF)
  (CL:LET* ((CURSOR SELF))
   (CL:LOOP
    (CL:COND
     ((SKOLEM? CURSOR)
      (CL:LET*
       ...
       (CL:IF (EQL? VALUE SELF)
        (CL:RETURN-FROM SKOLEM-WITH-VARIABLE-VALUE-CYCLE? CL:T)
        (CL:SETQ CURSOR VALUE))))
     (CL:T (CL:RETURN))))
   CL:NIL))

;;; (DEFUN SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES ...)

(CL:DEFUN SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES (FROMSKOLEM TOVALUE)
  (CL:COND
   ((JUST-IN-TIME-INFERENCE-CACHE? *CONTEXT*)
    (CL:LET*
     ((*CONTEXT*
       (GET-INFERENCE-CACHE (GET-WORLD-STATE *CONTEXT*)
        KWD-PROPOSITIONS-META))
      (*MODULE* (%BASE-MODULE *CONTEXT*)))
     ...
     (CL:LET*
      ...
      (CL:WHEN
       ...
       (CL:SETF (%VARIABLE-VALUE OBJECT-000) NEW-VALUE-000)))
     (CL:WHEN
      (CL:NOT (MEMB? (VARIABLE-VALUE-INVERSE TOVALUE) FROMSKOLEM))
      (CL:LET*
       ((OBJECT-001 TOVALUE)
        (VALUE-001 (CONS FROMSKOLEM (VARIABLE-VALUE-INVERSE TOVALUE)))
       ...
       (CL:WHEN
        (CL:NOT
         (CL:AND (CL:NOT (CL:EQ OLD-VALUE-001 NULL))
          (CL:EQ (PRIMARY-TYPE OLD-VALUE-001)
           SGT-PROPOSITIONS-STELLA-CS-VALUE)))
        (CL:SETF (%VARIABLE-VALUE-INVERSE OBJECT-001)
         NEW-VALUE-001))))))
   ((META-INFERENCE-CACHE? *CONTEXT*)
    (CL:LET*
     ((*CONTEXT*
       (GET-INFERENCE-CACHE (GET-WORLD-STATE *CONTEXT*)
        KWD-PROPOSITIONS-JUST-IN-TIME))
      (*MODULE* (%BASE-MODULE *CONTEXT*)))
     (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*))
     (CL:LET*
      ((OBJECT-002 FROMSKOLEM) (VALUE-002 TOVALUE)
     ...
     (CL:LET*
      ((INVERSES
        (ACCESS-IN-CONTEXT (%VARIABLE-VALUE-INVERSE TOVALUE)
         (%HOME-CONTEXT TOVALUE) CL:T)))
      (CL:WHEN (CL:EQ INVERSES NULL) (CL:SETQ INVERSES NIL))
      (CL:WHEN (CL:NOT (MEMB? INVERSES FROMSKOLEM))
       (CL:LET*
        ((OBJECT-003 TOVALUE) (VALUE-003 (CONS FROMSKOLEM INVERSES))
         (OLD-VALUE-004 (%VARIABLE-VALUE-INVERSE OBJECT-003))
         (NEW-VALUE-003
          (UPDATE-IN-CONTEXT OLD-VALUE-004 VALUE-003
           (%HOME-CONTEXT OBJECT-003) CL:NIL)))
        (CL:WHEN
...
(CL:DEFUN BIND-SKOLEM-TO-VALUE (FROMSKOLEM TOVALUE TOPLEVELUPDATE?)
  (VERIFY-SKOLEM-AND-VALUE-TYPES FROMSKOLEM TOVALUE)
  (CL:LET*
   ((OBJECT-000 FROMSKOLEM) (VALUE-000 TOVALUE)
    (OLD-VALUE-000 (%VARIABLE-VALUE OBJECT-000))
    (NEW-VALUE-000
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE TOVALUE)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPOSITIONS-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:WHEN (SKOLEM-WITH-VARIABLE-VALUE-CYCLE? TOVALUE)
       (CL:LET*
      ...
      (SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES FROMSKOLEM TOVALUE)
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPOSITIONS-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:OR
       (CL:NOT
        (CL:EQ
         (ACCESS-IN-CONTEXT (%VARIABLE-VALUE TERM) (%HOME-CONTEXT TERM)
          CL:NIL)
         NULL))
       (CL:NOT (CL:EQ (VARIABLE-VALUE-INVERSE TERM) NIL)))))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPOSITIONS-LOGIC-LOGIC-OBJECT)
     (CL:PROGN (CL:NOT (CL:EQ (VARIABLE-VALUE-INVERSE TERM) NIL))))
    (CL:T
     (CL:LET* ((FOUND?-000 CL:NIL))
      (CL:LET*
       ((PROP NULL)
...
;;; (DEFUN (GET-VARIABLE-VALUE-INVERSES CONS) ...)

(CL:DEFUN GET-VARIABLE-VALUE-INVERSES (TERM)
  (CL:LET* ((EQUIVS NIL))
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE TERM)
      SGT-PROPOSITIONS-LOGIC-LOGIC-OBJECT)
     (CL:PROGN (CL:SETQ EQUIVS (VARIABLE-VALUE-INVERSE TERM))))
    (CL:T
     (CL:LET*
      ((PROP NULL)
       (ITER-000
         ...
         (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
          (CL:SETQ ARG
           (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
            INDEX-000))
          (CL:WHEN
           (CL:AND (CL:NOT (EQL? ARG TERM))
            (CL:NOT (MEMBER? EQUIVS ARG)))
           (CL:IF (CL:EQ COLLECT-000 NULL)
            (CL:PROGN (CL:SETQ COLLECT-000 (CONS ARG NIL))
             (CL:IF (CL:EQ EQUIVS NIL) (CL:SETQ EQUIVS COLLECT-000)
              (ADD-CONS-TO-END-OF-CONS-LIST EQUIVS COLLECT-000)))
            (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS ARG NIL))
             (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
          (CL:SETQ INDEX-000 (CL:1+ INDEX-000)))))))))
   EQUIVS))

;;; (DEFUN (GET-ALL-VARIABLE-VALUE-INVERSES CONS) ...)

(CL:DEFUN GET-ALL-VARIABLE-VALUE-INVERSES (TERM)
  (CL:LET* ((EQUIVS NIL) (SEEDS (CONS TERM NIL)) (NEWEQUIVS NIL))
   (CL:LOOP WHILE (CL:NOT (CL:EQ SEEDS NIL)) DO
    (CL:LET* ((SEED NULL) (ITER-000 SEEDS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:SETQ SEED (%%VALUE ITER-000))
      (CL:LET*
       ((EQUIV NULL) (ITER-001 (GET-VARIABLE-VALUE-INVERSES SEED))
        (COLLECT-000 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:SETQ EQUIV (%%VALUE ITER-001))
        (CL:WHEN (CL:NOT (MEMBER? EQUIVS EQUIV))
         (CL:SETQ EQUIVS (CONS EQUIV EQUIVS))
         (CL:IF (CL:EQ COLLECT-000 NULL)
          (CL:PROGN (CL:SETQ COLLECT-000 (CONS EQUIV NIL))
           (CL:IF (CL:EQ NEWEQUIVS NIL) (CL:SETQ NEWEQUIVS COLLECT-000)
            (ADD-CONS-TO-END-OF-CONS-LIST NEWEQUIVS COLLECT-000)))
          (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS EQUIV NIL))
           (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
        (CL:SETQ ITER-001 (%%REST ITER-001))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
...
;;; (DEFUN (GET-ALL-EQUIVALENT-VALUES CONS) ...)

(CL:DEFUN GET-ALL-EQUIVALENT-VALUES (TERM)
  (CL:LET* ((REP (VALUE-OF TERM)))
   (CL:IF (EQL? REP TERM) (GET-ALL-VARIABLE-VALUE-INVERSES REP)
    (CONS REP (REMOVE (GET-ALL-VARIABLE-VALUE-INVERSES REP) TERM)))))
     ...
     (CL:IF (CL:EQ ARG NULL)
      ...
      (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE ARG)))
       (CL:COND
        ...
        ((SUBTYPE-OF-SURROGATE? TEST-VALUE-001)
         (CL:PROGN
          (CL:IF DEREFSURROGATES?
           (CL:SETQ ARGCODE (HASH-CODE (%SURROGATE-VALUE ARG)))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:COND
       ...
       ((CL:EQ OPTION KWD-PROPOSITIONS-SCORE)
           ...
           (CL:PROGN
            (CL:LET*
             ((OBJECT-000 PROPOSITION)
              (VALUE-000 (WRAP-FLOAT (COERCE-TO-FLOAT VALUE)))
              (OLD-VALUE-000
               (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OBJECT-000)
                SYM-PROPOSITIONS-LOGIC-SCORE NULL))
              (NEW-VALUE-000
               (UPDATE-IN-CONTEXT OLD-VALUE-000 VALUE-000
                (%HOME-CONTEXT OBJECT-000) CL:NIL)))
             (CL:WHEN
              (CL:NOT
               (CL:AND (CL:NOT (CL:EQ OLD-VALUE-000 NULL))
                (CL:EQ (PRIMARY-TYPE OLD-VALUE-000)
                 SGT-PROPOSITIONS-STELLA-CS-VALUE)))
              (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OBJECT-000)
               SYM-PROPOSITIONS-LOGIC-SCORE NEW-VALUE-000 NULL)))))
          ((SUBTYPE-OF-FLOAT? TEST-VALUE-002)
           (CL:PROGN
            (CL:LET*
             ((OBJECT-001 PROPOSITION)
              (VALUE-001 (WRAP-FLOAT (COERCE-TO-FLOAT VALUE)))
              (OLD-VALUE-001
               (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OBJECT-001)
                SYM-PROPOSITIONS-LOGIC-SCORE NULL))
              (NEW-VALUE-001
              ...
              (CL:NOT
               (CL:AND (CL:NOT (CL:EQ OLD-VALUE-001 NULL))
                (CL:EQ (PRIMARY-TYPE OLD-VALUE-001)
                 SGT-PROPOSITIONS-STELLA-CS-VALUE)))
              (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OBJECT-001)
               SYM-PROPOSITIONS-LOGIC-SCORE NEW-VALUE-001 NULL)))))
          (CL:T (REMOVE-AT ANNOTATIONS OPTION)
           (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
            (CL:LET* ((*PRINTREADABLY?* CL:T))
             (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
             (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003)
              "PARSING ERROR: Illegal value for :score option: `" VALUE
              "'." EOL)
...
;;; (DEFUN INHERIT-PROPOSITION-ANNOTATIONS ...)

(CL:DEFUN INHERIT-PROPOSITION-ANNOTATIONS (ANNOTATIONS TO)
  (CL:WHEN (CL:NOT (CL:EQ ANNOTATIONS NULL))
   (CL:LET*
    ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST ANNOTATIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ KEY (%%VALUE ITER-000))
     (CL:SETQ VALUE (%%VALUE (%%REST ITER-000)))
     (CL:WHEN (CL:EQ (LOOKUP-ANNOTATION TO KEY) NULL)
      (INSERT-ANNOTATION TO KEY VALUE))
     (CL:SETQ ITER-000 (%%REST (%%REST ITER-000)))))
   (FINALIZE-PROPOSITION-ANNOTATIONS TO)))
      ...
      (CL:LET*
       ...
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
         ...
         (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
       (CL:WHEN (CL:NOT (CL:EQ ANNOTATIONS NULL))
        (CL:LET* ((ARG NULL) (ITER-001 PROPOSITIONS))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:SETQ ARG (%%VALUE ITER-001))
          (INHERIT-PROPOSITION-ANNOTATIONS ANNOTATIONS ARG)
          (CL:SETQ ITER-001 (%%REST ITER-001)))))
      ...
      (CL:LET*
       ...
       (CL:COND
        ((CL:OR (CL:EQ UPDATEMODE KWD-PROPOSITIONS-ASSERT-TRUE)
         ...
         (INHERIT-PROPOSITION-ANNOTATIONS
          (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION)
           SYM-PROPOSITIONS-LOGIC-ANNOTATIONS NULL)
          ARGUMENT)
         (DESTROY-REDUNDANT-PROPOSITION PROPOSITION)
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION
   ...
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION-BACK-POINTER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NATIVE-RELATION-BACK-POINTER" NULL
     0))
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION-SETTER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NATIVE-RELATION-SETTER" NULL 0))
   ...
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-MATCH-SCORE
  ...
  (CL:PROGN
   ...
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-PROPOSITION-STRINGIFIED-SOURCE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PROPOSITION-STRINGIFIED-SOURCE"
     NULL 0))
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-PROPOSITION-NEURAL-NETWORK
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PROPOSITION-NEURAL-NETWORK" NULL
     0))
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-NEURAL-NETWORK
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NEURAL-NETWORK" NULL 0))
   (CL:SETQ SYM-PROPOSITIONS-LOGIC-NEURAL-NET
  ...
  (CL:PROGN
   ...
   (CL:SETQ SGT-PROPOSITIONS-STELLA-SLOT
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SLOT"
     (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ KWD-PROPOSITIONS-META
    (INTERN-RIGID-SYMBOL-WRT-MODULE "META" NULL 2))
   (CL:SETQ KWD-PROPOSITIONS-JUST-IN-TIME
   ...
   (CL:SETQ KWD-PROPOSITIONS-SCORE
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS6 ()
  (CL:PROGN (CL:SETQ ZERO-VARIABLES-VECTOR (NEW-VECTOR 0))
   ...
   (CL:LET* ((SELF-005 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-005) KWD-PROPOSITIONS-TRUE)
    (CL:SETF (%STRENGTH SELF-005) KWD-PROPOSITIONS-STRICT)
    (CL:SETF (%MATCH-SCORE SELF-005) 1.0d0)
    (CL:SETQ TRUE-TRUTH-VALUE SELF-005))
   (CL:LET* ((SELF-006 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-006) KWD-PROPOSITIONS-FALSE)
    (CL:SETF (%STRENGTH SELF-006) KWD-PROPOSITIONS-STRICT)
    (CL:SETF (%MATCH-SCORE SELF-006) -1.0d0)
    (CL:SETQ FALSE-TRUTH-VALUE SELF-006))
   (CL:LET* ((SELF-007 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-007) KWD-PROPOSITIONS-TRUE)
    (CL:SETF (%STRENGTH SELF-007) KWD-PROPOSITIONS-DEFAULT)
    (CL:SETF (%MATCH-SCORE SELF-007) 0.8d0)
    (CL:SETQ DEFAULT-TRUE-TRUTH-VALUE SELF-007))
   (CL:LET* ((SELF-008 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-008) KWD-PROPOSITIONS-FALSE)
    (CL:SETF (%STRENGTH SELF-008) KWD-PROPOSITIONS-DEFAULT)
    (CL:SETF (%MATCH-SCORE SELF-008) -0.8d0)
    (CL:SETQ DEFAULT-FALSE-TRUTH-VALUE SELF-008))
   (CL:LET* ((SELF-009 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-009) KWD-PROPOSITIONS-UNKNOWN)
    (CL:SETQ UNKNOWN-TRUTH-VALUE SELF-009))
   (CL:LET* ((SELF-010 (NEW-TRUTH-VALUE)))
    (CL:SETF (%POLARITY SELF-010) KWD-PROPOSITIONS-INCONSISTENT)
    (CL:SETF (%STRENGTH SELF-010) KWD-PROPOSITIONS-STRICT)
    (CL:SETQ INCONSISTENT-TRUTH-VALUE SELF-010))
   (CL:LET* ((SELF-011 (NEW-PROPOSITION)))
    (CL:SETF (%KIND SELF-011) KWD-PROPOSITIONS-CONSTANT)
    (CL:SETF (%OPERATOR SELF-011) SGT-PROPOSITIONS-STELLA-TRUE)
    (CL:SETF (%ARGUMENTS SELF-011) (NEW-VECTOR 0))
    (CL:LET*
     ((OBJECT-013 SELF-011) (VALUE-014 TRUE-TRUTH-VALUE)
      (OLD-VALUE-015 (%TRUTH-VALUE OBJECT-013))
      (NEW-VALUE-016
       (UPDATE-IN-CONTEXT OLD-VALUE-015 VALUE-014
        (%HOME-CONTEXT OBJECT-013) CL:NIL)))
     (CL:WHEN
      (CL:NOT
       (CL:AND (CL:NOT (CL:EQ OLD-VALUE-015 NULL))
        (CL:EQ (PRIMARY-TYPE OLD-VALUE-015)
         SGT-PROPOSITIONS-STELLA-CS-VALUE)))
      (CL:SETF (%TRUTH-VALUE OBJECT-013) NEW-VALUE-016)))
    (CL:SETF (%HOME-CONTEXT SELF-011) *PL-KERNEL-MODULE*)
    (CL:SETQ TRUE-PROPOSITION SELF-011))
   (CL:LET* ((SELF-017 (NEW-PROPOSITION)))
    (CL:SETF (%KIND SELF-017) KWD-PROPOSITIONS-CONSTANT)
    (CL:SETF (%OPERATOR SELF-017) SGT-PROPOSITIONS-STELLA-FALSE)
    (CL:SETF (%ARGUMENTS SELF-017) (NEW-VECTOR 0))
    (CL:LET*
     ((OBJECT-019 SELF-017) (VALUE-020 FALSE-TRUTH-VALUE)
      (OLD-VALUE-021 (%TRUTH-VALUE OBJECT-019))
      (NEW-VALUE-022
       (UPDATE-IN-CONTEXT OLD-VALUE-021 VALUE-020
        (%HOME-CONTEXT OBJECT-019) CL:NIL)))
     (CL:WHEN
      (CL:NOT
       (CL:AND (CL:NOT (CL:EQ OLD-VALUE-021 NULL))
        (CL:EQ (PRIMARY-TYPE OLD-VALUE-021)
         SGT-PROPOSITIONS-STELLA-CS-VALUE)))
      (CL:SETF (%TRUTH-VALUE OBJECT-019) NEW-VALUE-022)))
    (CL:SETF (%HOME-CONTEXT SELF-017) *PL-KERNEL-MODULE*)
    (CL:SETQ FALSE-PROPOSITION SELF-017))
   ...
   (CL:LET*
    ((CLASS
      (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "TRUTH-VALUE"
       "(DEFCLASS TRUTH-VALUE (LOGIC-OBJECT) :PUBLIC-SLOTS ((POLARITY :TYPE KEYWORD :INITIALLY :UNKNOWN :DOCUMENTATION \"One of :TRUE, :FALSE, :UNKNOWN or :INCONSISTENT.\") (STRENGTH :TYPE KEYWORD :INITIALLY NULL :DOCUMENTATION \"Either NULL, :STRICT or :DEFAULT.\") (MATCH-SCORE :TYPE PARTIAL-MATCH-SCORE :INITIALLY 0.0)))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS)
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS8 ()
  (CL:PROGN
   ...
   (DEFINE-INLINE-METHOD-CODE SGT-PROPOSITIONS-LOGIC-DESCRIPTION
    SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION
    (WRAP-METHOD-CODE (CL:FUNCTION NATIVE-RELATION)) NULL)
   (DEFINE-INLINE-METHOD-CODE SGT-PROPOSITIONS-LOGIC-NAMED-DESCRIPTION
    SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION
    (WRAP-METHOD-CODE (CL:FUNCTION NATIVE-RELATION)) NULL)
   (DEFINE-INLINE-METHOD-CODE SGT-PROPOSITIONS-LOGIC-NAMED-DESCRIPTION
    SYM-PROPOSITIONS-LOGIC-NATIVE-RELATION-SETTER
    (WRAP-METHOD-CODE (CL:FUNCTION NATIVE-RELATION-SETTER)) NULL)
   ...
   (DEFINE-EXTERNAL-SLOT-FROM-STRINGIFIED-SOURCE
    "(DEFSLOT CONTEXT MONOTONIC? :TYPE BOOLEAN :PUBLIC? TRUE :DOCUMENTATION \"Mark context as not allowing any retractions.\" :ALLOCATION :DYNAMIC)")
   (DEFINE-EXTERNAL-SLOT-FROM-STRINGIFIED-SOURCE
...
considering default assertions).\" :PUBLIC? TRUE)" (CL:FUNCTION FALSE?)
   ...
   (DEFINE-FUNCTION-OBJECT "ASSERTED-TRUE?"
    "(DEFUN (ASSERTED-TRUE? BOOLEAN) ((SELF PROPOSITION)) :DOCUMENTATION \"Return true if `self' has been asserted to be true.\" :PUBLIC? TRUE)"
    (CL:FUNCTION ASSERTED-TRUE?) NULL)
   (DEFINE-FUNCTION-OBJECT "ASSERTED-FALSE?"
    "(DEFUN (ASSERTED-FALSE? BOOLEAN) ((SELF PROPOSITION)) :DOCUMENTATION \"Return true if `self' has been asserted to be false.\" :PUBLIC? TRUE)"
    (CL:FUNCTION ASSERTED-FALSE?) NULL)
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS10 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "VERIFY-SKOLEM-AND-VALUE-TYPES"
    "(DEFUN VERIFY-SKOLEM-AND-VALUE-TYPES ((FROMSKOLEM SKOLEM) (TOVALUE OBJECT)))"
    (CL:FUNCTION VERIFY-SKOLEM-AND-VALUE-TYPES) NULL)
   (DEFINE-FUNCTION-OBJECT "SKOLEM-WITH-VARIABLE-VALUE-CYCLE?"
    "(DEFUN (SKOLEM-WITH-VARIABLE-VALUE-CYCLE? BOOLEAN) ((SELF OBJECT)))"
    (CL:FUNCTION SKOLEM-WITH-VARIABLE-VALUE-CYCLE?) NULL)
   (DEFINE-FUNCTION-OBJECT
    "SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES"
    "(DEFUN SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES ((FROMSKOLEM SKOLEM) (TOVALUE SKOLEM)))"
    (CL:FUNCTION SYNCHRONIZE-VARIABLE-VALUE-INFERENCE-CACHES) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "HAS-EQUIVALENT-VALUES?"
    "(DEFUN (HAS-EQUIVALENT-VALUES? BOOLEAN) ((TERM OBJECT)))"
    (CL:FUNCTION HAS-EQUIVALENT-VALUES?) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-VARIABLE-VALUE-INVERSES"
    "(DEFUN (GET-VARIABLE-VALUE-INVERSES CONS) ((TERM OBJECT)))"
    (CL:FUNCTION GET-VARIABLE-VALUE-INVERSES) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-ALL-VARIABLE-VALUE-INVERSES"
    "(DEFUN (GET-ALL-VARIABLE-VALUE-INVERSES CONS) ((TERM OBJECT)))"
    (CL:FUNCTION GET-ALL-VARIABLE-VALUE-INVERSES) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-ALL-EQUIVALENT-VALUES"
    "(DEFUN (GET-ALL-EQUIVALENT-VALUES CONS) ((TERM OBJECT)))"
    (CL:FUNCTION GET-ALL-EQUIVALENT-VALUES) NULL)
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS11 ()
  (CL:PROGN
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION DESCRIPTION-SURROGATE)) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "INHERIT-PROPOSITION-ANNOTATIONS"
    "(DEFUN INHERIT-PROPOSITION-ANNOTATIONS ((ANNOTATIONS PROPERTY-LIST) (TO PROPOSITION)))"
    (CL:FUNCTION INHERIT-PROPOSITION-ANNOTATIONS) NULL)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     ...
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *PARTIAL-SUPPORT-COUNTER* INTEGER 0)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
...
call 'bound-to' safely.\")" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *SKOLEM-ID-COUNTER* INTEGER 0)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFSPECIAL *VARIABLEIDCOUNTER* INTEGER 0)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFSPECIAL *FREESKOLEMS* CONS NULL :DOCUMENTATION \"Cons-list of top-level existentially-quantified skolems.\")"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFCONSTANT TRUE-TRUTH-VALUE TRUTH-VALUE (NEW TRUTH-VALUE :POLARITY :TRUE :STRENGTH :STRICT :MATCH-SCORE 1.0) :DOCUMENTATION \"Value representing TRUE.\")"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFCONSTANT FALSE-TRUTH-VALUE TRUTH-VALUE (NEW TRUTH-VALUE :POLARITY :FALSE :STRENGTH :STRICT :MATCH-SCORE -1.0) :DOCUMENTATION \"Value representing FALSE.\")"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFCONSTANT DEFAULT-TRUE-TRUTH-VALUE TRUTH-VALUE (NEW TRUTH-VALUE :POLARITY :TRUE :STRENGTH :DEFAULT :MATCH-SCORE 0.8) :DOCUMENTATION \"Value representing DEFAULT-TRUE.\")"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFCONSTANT DEFAULT-FALSE-TRUTH-VALUE TRUTH-VALUE (NEW TRUTH-VALUE :POLARITY :FALSE :STRENGTH :DEFAULT :MATCH-SCORE -0.8) :DOCUMENTATION \"Value representing DEFAULT-FALSE.\")"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
...
record current bindings of automatic instance symbols.\")" NULL)
    ...
    (ADD-HOOK *REDEFINE-RELATION-HOOKS*
     SYM-PROPOSITIONS-LOGIC-TRANSFER-LOGIC-INFORMATION-FROM-RELATION-HOOK)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFSPECIAL *DEFAULTCREATIONTYPE* SURROGATE NULL)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
...
but might pass once finalization is complete.\")" NULL)
    ...
    (REGISTER-NATIVE-NAME SYM-PROPOSITIONS-LOGIC-UNASSERT
     KWD-PROPOSITIONS-COMMON-LISP KWD-PROPOSITIONS-FUNCTION)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFSPECIAL *DEFERRED-TERMS-TO-UNLINK* HASH-SET NULL)" NULL)

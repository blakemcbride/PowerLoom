...
(CL:DEFVAR SYM-JUSTIFICATIONS-LOGIC-INFERENCE-STRATEGY NULL)
(CL:DEFVAR KWD-JUSTIFICATIONS-BACKWARD NULL)
(CL:DEFVAR SYM-JUSTIFICATIONS-LOGIC-INFERENCE-DIRECTION NULL)
...
(CL:DEFVAR SYM-JUSTIFICATIONS-LOGIC-ERROR-SCORE NULL)
...
(CL:DEFVAR KWD-JUSTIFICATIONS-MATCH-SCORE NULL)
...
(CL:DEFVAR SYM-JUSTIFICATIONS-STELLA-DELETED? NULL)
(CL:DEFVAR SYM-JUSTIFICATIONS-LOGIC-SUPPORTED? NULL)
(CL:DEFVAR SYM-JUSTIFICATIONS-LOGIC-SUPPORTING-CONTEXTS NULL)
...
(CL:DEFVAR SGT-JUSTIFICATIONS-PL-KERNEL-KB-SUBSET-OF NULL)
(CL:DEFVAR SGT-JUSTIFICATIONS-PL-KERNEL-KB-EQUIVALENT NULL)
...
proposition of this justification." :ALLOCATION :INSTANCE :ACCESSOR
   ...
   (MATCH-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT
    :DOCUMENTATION
    "Positive partial match score of the derived proposition."
    :ALLOCATION :INSTANCE :ACCESSOR %MATCH-SCORE)
   (ERROR-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT
    :DOCUMENTATION "Error value used by partial matcher" :ALLOCATION
    :INSTANCE :ACCESSOR %ERROR-SCORE)))
...
(CL:DEFUN NEW-JUSTIFICATION ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%ERROR-SCORE SELF) NULL-FLOAT) SELF))
  ...
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-POSITIVE-SCORE)
    (CL:IF SETVALUE?
     (CL:SETF (%MATCH-SCORE SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-FLOAT (%MATCH-SCORE SELF)))))
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-ERROR-SCORE)
    (CL:IF SETVALUE?
     (CL:SETF (%ERROR-SCORE SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-FLOAT (%ERROR-SCORE SELF)))))
   (CL:T
...
(CL:DEFUN NEW-PRIMITIVE-STRATEGY ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%SUBSTITUTION SELF) NULL)
   (CL:SETF (%ANTECEDENTS SELF) NIL) (CL:SETF (%PROPOSITION SELF) NULL)
   (CL:SETF (%ERROR-SCORE SELF) NULL-FLOAT)
...
as propositions and substitutions."
  (CL:LET* ((COPY NULL) (ANTECEDENTS NIL))
   ...
   (CL:SETF (%MATCH-SCORE COPY) (%MATCH-SCORE SELF))
   (CL:SETF (%ERROR-SCORE COPY) (%ERROR-SCORE SELF))
   (CL:LET*
    ...
    (CL:LET*
     ...
     (CL:COND
      ((CL:AND (CL:NOT (CL:EQ (%JUSTIFICATIONS FRAME) NULL))
        ...
        (ISA? (FIRST (%JUSTIFICATIONS FRAME))
         SGT-JUSTIFICATIONS-LOGIC-JUSTIFICATION))
      ...
      ((CL:NOT (CL:EQ (%RESULT FRAME) NULL))
       (CL:COND
        ...
        ((CL:AND (CL:NOT (CL:EQ (%PROPOSITION (%RESULT FRAME)) NULL))
          (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS?
           (%PROPOSITION (%RESULT FRAME))))
         (CL:SETQ JUSTIFICATION
          (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION
           (%PROPOSITION (%RESULT FRAME)))))))
      ((CL:AND (CL:NOT (CL:EQ (%PROPOSITION FRAME) NULL))
        (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? (%PROPOSITION FRAME)))
  ...
  (CL:LET* ((*EXPLANATION-STYLE* STYLE))
   ...
   (CL:LET*
    ...
    (CL:IF (CL:NOT (CL:EQ (%TRUTH-VALUE SELF) NULL))
     ...
     (INSERT-AT KEYS KWD-JUSTIFICATIONS-TRUTH-VALUE
      UNKNOWN-TRUTH-VALUE))
    (CL:WHEN (CL:NOT (CL:= (%MATCH-SCORE SELF) NULL-FLOAT))
     (INSERT-AT KEYS KWD-JUSTIFICATIONS-MATCH-SCORE
      (WRAP-FLOAT (%MATCH-SCORE SELF))))
...
(CL:DEFCLASS FORWARD-JUSTIFICATION (JUSTIFICATION)
  ((SUPPORTING-CONTEXTS :DOCUMENTATION
    "Lists contexts in which the validity of this justification has been checked."
    :ALLOCATION :INSTANCE :ACCESSOR %SUPPORTING-CONTEXTS)))
...
(CL:DEFUN NEW-FORWARD-JUSTIFICATION ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%INFERENCE-RULE SELF) NULL)
   (CL:SETF (%ERROR-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%SUPPORTING-CONTEXTS SELF) (LIST *CONTEXT*)) SELF))
...
(CL:DEFMETHOD DELETED? ((SELF FORWARD-JUSTIFICATION))
  (CL:OR
   (CL:AND (CL:NOT (CL:EQ (%PROPOSITION SELF) NULL))
    (DELETED? (%PROPOSITION SELF)))
   (EMPTY? (REMOVE-DELETED-MEMBERS (%SUPPORTING-CONTEXTS SELF)))))

(CL:DEFMETHOD SUPPORTED? ((SELF FORWARD-JUSTIFICATION))
  (CL:LET* ((FOUND?-000 CL:NIL))
   (CL:LET*
    ((CTXT NULL)
     (ITER-000 (%THE-CONS-LIST (%SUPPORTING-CONTEXTS SELF))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ CTXT (%%VALUE ITER-000))
     (CL:WHEN (VISIBLE-FROM? CTXT *CONTEXT*) (CL:SETQ FOUND?-000 CL:T)
      (CL:RETURN))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN ACCESS-FORWARD-JUSTIFICATION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-SUPPORTING-CONTEXTS)
    (CL:IF SETVALUE? (CL:SETF (%SUPPORTING-CONTEXTS SELF) VALUE)
     (CL:SETQ VALUE (%SUPPORTING-CONTEXTS SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
...
(CL:DEFUN HAS-FORWARD-JUSTIFICATIONS? (PROPOSITION)
  ...
  (NON-EMPTY?
   (REMOVE-DELETED-MEMBERS (FORWARD-JUSTIFICATIONS PROPOSITION))))

;;; (DEFUN (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? BOOLEAN) ...)

(CL:DEFUN HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? (PROPOSITION)
  ...
  (CL:LET* ((FOUND?-000 CL:NIL))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (SUPPORTED? JUST) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((VALUE-000 FOUND?-000)) VALUE-000)))

;;; (DEFUN (GET-FORWARD-JUSTIFICATIONS (LIST OF FORWARD-JUSTIFICATION)) ...)

(CL:DEFUN GET-FORWARD-JUSTIFICATIONS (PROPOSITION)
  "Return `proposition's forward justifications."
  (REMOVE-DELETED-MEMBERS (FORWARD-JUSTIFICATIONS PROPOSITION)))

;;; (DEFUN (GET-SUPPORTED-FORWARD-JUSTIFICATIONS (LIST OF FORWARD-JUSTIFICATION)) ...)

(CL:DEFUN GET-SUPPORTED-FORWARD-JUSTIFICATIONS (PROPOSITION)
  ...
  (CL:LET*
   ...
   (CL:LET* ((FOUND?-000 CL:NIL))
    (CL:LET* ((JUST NULL) (ITER-000 (%THE-CONS-LIST JUSTIFICATIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:SETQ JUST (%%VALUE ITER-000))
      (CL:WHEN (CL:NOT (SUPPORTED? JUST)) (CL:SETQ FOUND?-000 CL:T)
       (CL:RETURN))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (CL:IF FOUND?-000
     (CL:PROGN
      (CL:LET* ((SUPPORTED (NEW-LIST)))
       (CL:LET*
        ((JUST NULL) (ITER-001 (%THE-CONS-LIST JUSTIFICATIONS))
         (COLLECT-000 NULL))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
         (CL:SETQ JUST (%%VALUE ITER-001))
         (CL:WHEN (SUPPORTED? JUST)
          (CL:IF (CL:EQ COLLECT-000 NULL)
           (CL:PROGN (CL:SETQ COLLECT-000 (CONS JUST NIL))
            (CL:IF (CL:EQ (%THE-CONS-LIST SUPPORTED) NIL)
             (CL:SETF (%THE-CONS-LIST SUPPORTED) COLLECT-000)
             (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST SUPPORTED)
              COLLECT-000)))
           (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS JUST NIL))
            (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
         (CL:SETQ ITER-001 (%%REST ITER-001))))
       SUPPORTED))
     (CL:PROGN JUSTIFICATIONS)))))

;;; (DEFUN (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION FORWARD-JUSTIFICATION) ...)

(CL:DEFUN GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION (PROPOSITION)
  ...
  (CL:LET* ((VALUE-000 NULL))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ JUST (%%VALUE ITER-000))
     (CL:WHEN (SUPPORTED? JUST) (CL:SETQ VALUE-000 JUST) (CL:RETURN))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((VALUE-001 VALUE-000)) VALUE-001)))

;;; (DEFUN ADD-FORWARD-JUSTIFICATION ...)

(CL:DEFUN ADD-FORWARD-JUSTIFICATION (PROPOSITION JUSTIFICATION)
  (CL:WHEN
   (CL:NOT
    (ISA? JUSTIFICATION
     SGT-JUSTIFICATIONS-LOGIC-FORWARD-JUSTIFICATION))
   (CL:ERROR
    "Safety violation: INTERNAL ERROR: forward justification is of the wrong type"))
  (CL:LET*
   ((JUSTIFICATIONS
     (REMOVE-DELETED-MEMBERS (FORWARD-JUSTIFICATIONS PROPOSITION))))
   (CL:LET* ((FOUND?-000 CL:NIL))
    (CL:LET* ((JUST NULL) (ITER-000 (%THE-CONS-LIST JUSTIFICATIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (CL:WHEN FOUND?-000 (CL:RETURN-FROM ADD-FORWARD-JUSTIFICATION)))
...
;;; (DEFUN (CREATE-FORWARD-RULE-JUSTIFICATION FORWARD-JUSTIFICATION) ...)

(CL:DEFUN CREATE-FORWARD-RULE-JUSTIFICATION (ANTECEDENTS FORWARDRULE ARGUMENTS CONSEQUENTPROPOSITION BC-JUSTIFICATION)
  (CL:LET* ((SELF-000 (NEW-FORWARD-JUSTIFICATION)))
   ...
   (CL:LET*
    ...
    (CL:COND
     ((CL:NOT (CL:EQ BC-JUSTIFICATION NULL))
      (CL:SETQ SUBSTITUTION (%SUBSTITUTION BC-JUSTIFICATION)))
     (CL:T (CL:SETQ SUBSTITUTION (NEW-KEY-VALUE-MAP))
      (CL:LET*
       ((ARG NULL) (VECTOR-000 ARGUMENTS) (INDEX-000 0)
       ...
       (CL:LOOP WHILE
        ...
        (CL:SETQ INDEX-000 (CL:1+ INDEX-000))
        (CL:SETQ INDEX-001 (CL:1+ INDEX-001))))))
    (CL:WHEN (CL:NOT (CL:EQ BC-JUSTIFICATION NULL))
...
(CL:DEFUN GET-FORWARD-ANTECEDENT-JUSTIFICATION (ANTECEDENT)
  (CL:IF (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? ANTECEDENT)
   (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION ANTECEDENT)
   (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY)))
...
;;; (DEFUN RECORD-FORWARD-RULE-JUSTIFICATION ...)

(CL:DEFUN RECORD-FORWARD-RULE-JUSTIFICATION (ANTECEDENTS FORWARDRULE ARGUMENTS CONSEQUENTPROPOSITION BC-JUSTIFICATION)
  (CL:WHEN (CL:NOT *RECORD-JUSTIFICATIONS?*)
   (CL:RETURN-FROM RECORD-FORWARD-RULE-JUSTIFICATION))
  (CL:LET*
   ((JUSTIFICATIONS
     (REMOVE-DELETED-MEMBERS
      (FORWARD-JUSTIFICATIONS CONSEQUENTPROPOSITION)))
    (NEW-FJ
     (CREATE-FORWARD-RULE-JUSTIFICATION ANTECEDENTS FORWARDRULE
      ARGUMENTS CONSEQUENTPROPOSITION BC-JUSTIFICATION)))
   (CL:LET* ((FJ NULL) (ITER-000 (%THE-CONS-LIST JUSTIFICATIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      ...
      (CL:RETURN-FROM RECORD-FORWARD-RULE-JUSTIFICATION))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (ADD-FORWARD-JUSTIFICATION CONSEQUENTPROPOSITION NEW-FJ)))

;;; (DEFUN (CREATE-SUBSET-OF-JUSTIFICATION JUSTIFICATION) ...)

(CL:DEFUN CREATE-SUBSET-OF-JUSTIFICATION (SUB SUPER)
  (CL:SETQ SUB (GET-DESCRIPTION SUB))
  (CL:SETQ SUPER (GET-DESCRIPTION SUPER))
  (CL:COND
   ((CL:AND (CL:NOT (CL:EQ SUB NULL)) (CL:NOT (CL:EQ SUPER NULL))
     (COLLECTION-IMPLIES-COLLECTION? SUB SUPER))
    (CL:LET*
     ((SUBSETPROP
       (%PROPOSITION
        (SURROGATE-TO-DESCRIPTION
         SGT-JUSTIFICATIONS-PL-KERNEL-KB-SUBSET-OF)))
      (SUBSETARGS (%ARGUMENTS SUBSETPROP))
      (SUBSTITUTION (NEW-KEY-VALUE-MAP)))
     (INSERT-AT SUBSTITUTION
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SUBSETARGS)) 1)
      SUPER)
     (INSERT-AT SUBSTITUTION
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SUBSETARGS)) 0)
      SUB)
     (CL:LET* ((SELF-001 (NEW-PRIMITIVE-STRATEGY)))
      (CL:SETF (%PROPOSITION SELF-001) SUBSETPROP)
      (CL:SETF (%SUBSTITUTION SELF-001) SUBSTITUTION)
      (CL:SETF (%TRUTH-VALUE SELF-001) TRUE-TRUTH-VALUE)
      (CL:SETF (%STRATEGY SELF-001) KWD-JUSTIFICATIONS-SPECIALIST)
      (CL:LET* ((VALUE-000 SELF-001)) VALUE-000))))
   (CL:T NULL)))

;;; (DEFUN (GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS (CONS OF PROPOSITION)) ...)

(CL:DEFUN GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS (X Y)
  (CL:SETQ Y Y)
  (CL:LET*
   ((EQUIVTERMS (CONS X (GET-ALL-EQUIVALENT-VALUES X)))
    (EQUIVPROPS NIL) (EXPLICITEQUIVPROPS NIL)
    (WORLDSTATE (GET-WORLD-STATE *CONTEXT*)))
   (CL:LET* ((TERM NULL) (ITER-000 EQUIVTERMS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET*
      ((PROP NULL)
       (ITER-001
        (ALLOCATE-ITERATOR
         (UNFILTERED-DEPENDENT-PROPOSITIONS TERM
          SGT-JUSTIFICATIONS-PL-KERNEL-KB-EQUIVALENT))))
      (CL:LOOP WHILE (NEXT? ITER-001) DO
       ...
       (CL:WHEN
        ...
        (CL:LET*
         ...
         (CL:WHEN
          (CL:OR (CL:AND (EQL? ARG1 TERM) (MEMBER? EQUIVTERMS ARG2))
           (CL:AND (EQL? ARG2 TERM) (MEMBER? EQUIVTERMS ARG1)))
          (CL:SETQ EQUIVPROPS (CONS PROP EQUIVPROPS)))))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((PROP NULL) (ITER-002 EQUIVPROPS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
     (CL:SETQ PROP (%%VALUE ITER-002))
     (CL:TAGBODY
      (CL:WHEN (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? PROP)
       (CL:SETQ EXPLICITEQUIVPROPS (CONS PROP EXPLICITEQUIVPROPS))
       (CL:GO :CONTINUE))
      (CL:LET*
       ((*CONTEXT* WORLDSTATE) (*MODULE* (%BASE-MODULE *CONTEXT*)))
      ...
      :CONTINUE)
     (CL:SETQ ITER-002 (%%REST ITER-002))))
   (CL:WHEN (CL:NOT (CL:EQ EXPLICITEQUIVPROPS NIL))
    (CL:SETQ EQUIVPROPS EXPLICITEQUIVPROPS))
   (FIND-EQUIVALENCE-PROPOSITION-PATH EQUIVPROPS X Y)))

;;; (DEFUN (FIND-EQUIVALENCE-PROPOSITION-PATH (CONS OF PROPOSITION)) ...)

(CL:DEFUN FIND-EQUIVALENCE-PROPOSITION-PATH (EQUIVPROPS X Y)
  (CL:LET*
   ((EDGETABLE (NEW-KEY-VALUE-MAP)) (ALLPATHS (CONS (CONS X NIL) NIL))
    (NEWPATHS NIL) (NPATHS 0) (MAXPATHS 1000))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NPATHS MAXPATHS))
   (CL:WHEN (CL:EQ EQUIVPROPS NIL)
    (CL:RETURN-FROM FIND-EQUIVALENCE-PROPOSITION-PATH NIL))
   (CL:LET* ((PROP NULL) (ITER-000 EQUIVPROPS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       (CL:SETQ ARG
        (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
         INDEX-000))
       (CL:LET* ((EDGES (LOOKUP EDGETABLE ARG)))
        (CL:WHEN (CL:EQ EDGES NULL) (CL:SETQ EDGES NIL))
        (CL:SETQ EDGES (CONS PROP EDGES))
        (INSERT-AT EDGETABLE ARG EDGES))
       (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LOOP (CL:SETQ NPATHS 0)
    (CL:LET* ((PATH NULL) (ITER-001 ALLPATHS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
      (CL:SETQ PATH (%%VALUE ITER-001))
      (CL:LET*
       ((PROP NULL) (ITER-002 (LOOKUP EDGETABLE (%%VALUE PATH))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
        (CL:SETQ PROP (%%VALUE ITER-002))
        (CL:LET*
         ...
         (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
          (CL:SETQ ARG
           (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001))
            INDEX-001))
          (CL:TAGBODY
           (CL:WHEN (CL:NOT (MEMBER? PATH ARG))
            (CL:LET* ((CAND (CONS ARG PATH)))
             (CL:LET* ((FOUND?-000 CL:NIL))
              (CL:LET* ((P NULL) (ITER-003 NEWPATHS))
               (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
                (CL:SETQ P (%%VALUE ITER-003))
                (CL:WHEN (EQUAL? P CAND) (CL:SETQ FOUND?-000 CL:T)
                 (CL:RETURN))
                (CL:SETQ ITER-003 (%%REST ITER-003))))
              (CL:IF FOUND?-000 (CL:PROGN (CL:GO :CONTINUE))
               (CL:IF (EQL? ARG Y)
                (CL:PROGN
                 (CL:LET* ((PROPS NIL)) (CL:SETQ PATH CAND)
                  (CL:LOOP WHILE (CL:NOT (CL:EQ (%%REST PATH) NIL)) DO
                   (CL:LET*
                    ((PROP NULL)
                     (ITER-004 (LOOKUP EDGETABLE (%%VALUE PATH))))
                    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-004 NIL)) DO
                     (CL:SETQ PROP (%%VALUE ITER-004))
                     (CL:LET* ((FOUND?-001 CL:NIL))
                      (CL:LET*
                       ((ARG NULL) (VECTOR-002 (%ARGUMENTS PROP))
                       ...
                       (CL:LOOP WHILE (CL:< INDEX-002 LENGTH-002) DO
                        (CL:SETQ ARG
                         (CL:AREF
                          (CL:THE CL:SIMPLE-VECTOR
                           (%THE-ARRAY VECTOR-002))
                          INDEX-002))
                        (CL:WHEN (EQL? ARG (%%VALUE (%%REST PATH)))
                         (CL:SETQ FOUND?-001 CL:T) (CL:RETURN))
                        (CL:SETQ INDEX-002 (CL:1+ INDEX-002))))
                      (CL:WHEN FOUND?-001
                       (CL:SETQ PROPS (CONS PROP PROPS)) (CL:RETURN)))
                     (CL:SETQ ITER-004 (%%REST ITER-004))))
                   (CL:SETQ PATH (%%REST PATH)))
                  (CL:RETURN-FROM FIND-EQUIVALENCE-PROPOSITION-PATH
                   PROPS)))
                (CL:PROGN (CL:SETQ NEWPATHS (CONS CAND NEWPATHS))
                 (CL:SETQ NPATHS (CL:1+ NPATHS))
                 (CL:WHEN (CL:>= NPATHS MAXPATHS)
                  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
                   "find-equivalence-proposition-path: CUTOFF" EOL)
                  (CL:RETURN-FROM FIND-EQUIVALENCE-PROPOSITION-PATH
                   EQUIVPROPS))))))))
           :CONTINUE)
          (CL:SETQ INDEX-001 (CL:1+ INDEX-001))))
    ...
    (CL:COND
     ((CL:= NPATHS 0)
      (CL:RETURN-FROM FIND-EQUIVALENCE-PROPOSITION-PATH EQUIVPROPS))
     (CL:T (CL:SETQ ALLPATHS NEWPATHS) (CL:SETQ NEWPATHS NIL))))))

;;; (DEFUN (CREATE-EQUIVALENCE-JUSTIFICATIONS (CONS OF JUSTIFICATION)) ...)

(CL:DEFUN CREATE-EQUIVALENCE-JUSTIFICATIONS (X Y)
  (CL:LET* ((XVALUE X) (YVALUE Y))
        ...
        (CL:AREF
         (CL:THE CL:SIMPLE-VECTOR
          (%THE-ARRAY
           (%VARIABLE-BINDINGS
            (%CURRENT-PATTERN-RECORD *QUERYITERATOR*))))
         (CL:THE CL:FIXNUM (%BOUND-TO-OFFSET X))))))
     ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ XVALUE (%SURROGATE-VALUE X))))
     (CL:T)))
        ...
        (CL:AREF
         (CL:THE CL:SIMPLE-VECTOR
          (%THE-ARRAY
           (%VARIABLE-BINDINGS
            (%CURRENT-PATTERN-RECORD *QUERYITERATOR*))))
         (CL:THE CL:FIXNUM (%BOUND-TO-OFFSET Y))))))
     ((SUBTYPE-OF-SURROGATE? TEST-VALUE-001)
      (CL:PROGN (CL:SETQ YVALUE (%SURROGATE-VALUE Y))))
     (CL:T)))
   (CL:WHEN (EQL? XVALUE YVALUE)
    (CL:RETURN-FROM CREATE-EQUIVALENCE-JUSTIFICATIONS NIL))
   (CL:WHEN (CL:NOT (SKOLEM? XVALUE))
    (CL:LET* ((AUX XVALUE)) (CL:SETQ XVALUE YVALUE)
     (CL:SETQ YVALUE AUX)))
   (CL:WHEN
    (CL:OR (CL:NOT (SKOLEM? XVALUE))
     (CL:NOT (EQL? (VALUE-OF XVALUE) (VALUE-OF YVALUE))))
    (CL:RETURN-FROM CREATE-EQUIVALENCE-JUSTIFICATIONS NIL))
   (CL:LET* ((JUSTIFICATIONS NIL))
    (CL:LET*
     ((EQUIVPROP NULL)
      (ITER-000
       (GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS XVALUE YVALUE)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:SETQ EQUIVPROP (%%VALUE ITER-000))
      (CL:COND
       ((HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? EQUIVPROP)
        (CL:SETQ JUSTIFICATIONS
         (CONS (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION EQUIVPROP)
          JUSTIFICATIONS)))
       (CL:T
        (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY)))
         (CL:SETF (%PROPOSITION SELF-000) EQUIVPROP)
         (CL:SETF (%TRUTH-VALUE SELF-000)
          (ACCESS-IN-CONTEXT (%TRUTH-VALUE EQUIVPROP)
           (%HOME-CONTEXT EQUIVPROP) CL:NIL))
         (CL:SETF (%STRATEGY SELF-000)
          KWD-JUSTIFICATIONS-LOOKUP-ASSERTIONS)
         (CL:SETQ JUSTIFICATIONS (CONS SELF-000 JUSTIFICATIONS)))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    JUSTIFICATIONS)))

;;; (DEFUN (CREATE-IMPLICIT-INFERENCE-JUSTIFICATION JUSTIFICATION) ...)

(CL:DEFUN CREATE-IMPLICIT-INFERENCE-JUSTIFICATION (INFERREDPROP SUPPORTPROP)
  (CL:LET*
   ((INFERREDOP (%OPERATOR INFERREDPROP))
    (SUPPORTOP (%OPERATOR SUPPORTPROP)) (JUSTIFICATIONS NIL))
   (CL:WHEN (CL:NOT (CL:EQ INFERREDOP SUPPORTOP))
    (CL:LET*
     ((SUBSETJUST
       (CREATE-SUBSET-OF-JUSTIFICATION SUPPORTOP INFERREDOP)))
     (CL:WHEN (CL:NOT (CL:EQ SUBSETJUST NULL))
      (CL:SETQ JUSTIFICATIONS (CONS SUBSETJUST JUSTIFICATIONS)))))
   (CL:LET*
    ((INFARG NULL) (VECTOR-000 (%ARGUMENTS INFERREDPROP)) (INDEX-000 0)
     (LENGTH-000 (LENGTH VECTOR-000)) (SUPARG NULL)
     (VECTOR-001 (%ARGUMENTS SUPPORTPROP)) (INDEX-001 0)
     (LENGTH-001 (LENGTH VECTOR-001)))
    (CL:DECLARE
     (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 INDEX-001 LENGTH-001))
    (CL:LOOP WHILE
     ...
     (CL:LET*
      ((EQUIVJUST NULL)
       (ITER-000 (CREATE-EQUIVALENCE-JUSTIFICATIONS INFARG SUPARG)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:SETQ EQUIVJUST (%%VALUE ITER-000))
       (CL:LET* ((TEST-VALUE-000 CL:NIL))
        (CL:LET* ((FOUND?-000 CL:NIL))
         (CL:LET* ((JUST NULL) (ITER-001 JUSTIFICATIONS))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
           (CL:SETQ JUST (%%VALUE ITER-001))
           (CL:WHEN (JUSTIFICATION-EQL? EQUIVJUST JUST)
            (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
           (CL:SETQ ITER-001 (%%REST ITER-001))))
         (CL:SETQ TEST-VALUE-000 FOUND?-000))
        (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000))
        (CL:WHEN TEST-VALUE-000
         (CL:SETQ JUSTIFICATIONS (CONS EQUIVJUST JUSTIFICATIONS))))
       (CL:SETQ ITER-000 (%%REST ITER-000))))
     (CL:SETQ INDEX-000 (CL:1+ INDEX-000))
     (CL:SETQ INDEX-001 (CL:1+ INDEX-001))))
   (CL:COND
    ((CL:NOT (CL:EQ JUSTIFICATIONS NIL))
     (CL:COND
      ((HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? SUPPORTPROP)
       (CL:SETQ JUSTIFICATIONS
        (CONS (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION SUPPORTPROP)
         JUSTIFICATIONS)))
      (CL:T
       (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY)))
        (CL:SETF (%PROPOSITION SELF-000) SUPPORTPROP)
        (CL:SETF (%STRATEGY SELF-000)
         KWD-JUSTIFICATIONS-LOOKUP-ASSERTIONS)
        (CL:SETQ JUSTIFICATIONS (CONS SELF-000 JUSTIFICATIONS)))))
     (CL:LET* ((SELF-001 (NEW-JUSTIFICATION)))
      ...
      (CL:SETF (%PROPOSITION SELF-001) INFERREDPROP)
      (CL:SETF (%ANTECEDENTS SELF-001) JUSTIFICATIONS)
      (CL:LET* ((VALUE-000 SELF-001)) VALUE-000)))
    (CL:T NULL))))

;;; (DEFCLASS CLASH-JUSTIFICATION ...)

(CL:DEFCLASS CLASH-JUSTIFICATION (FORWARD-JUSTIFICATION)
...
(CL:DEFUN NEW-CLASH-JUSTIFICATION ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%INFERENCE-RULE SELF) NULL)
   (CL:SETF (%ERROR-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%SUPPORTING-CONTEXTS SELF) (LIST *CONTEXT*))
...
(CL:DEFUN HELP-STARTUP-JUSTIFICATIONS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-JUSTIFICATIONS-LOGIC-INFERENCE-STRATEGY
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INFERENCE-STRATEGY" NULL 0))
   (CL:SETQ KWD-JUSTIFICATIONS-BACKWARD
    (INTERN-RIGID-SYMBOL-WRT-MODULE "BACKWARD" NULL 2))
   (CL:SETQ SYM-JUSTIFICATIONS-LOGIC-INFERENCE-DIRECTION
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INFERENCE-DIRECTION" NULL 0))
   ...
   (CL:SETQ SYM-JUSTIFICATIONS-LOGIC-ERROR-SCORE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ERROR-SCORE" NULL 0))
   ...
   (CL:SETQ KWD-JUSTIFICATIONS-MATCH-SCORE
...
(CL:DEFUN HELP-STARTUP-JUSTIFICATIONS2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-JUSTIFICATIONS-STELLA-DELETED?
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DELETED?"
     (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   (CL:SETQ SYM-JUSTIFICATIONS-LOGIC-SUPPORTED?
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SUPPORTED?" NULL 0))
   (CL:SETQ SYM-JUSTIFICATIONS-LOGIC-SUPPORTING-CONTEXTS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SUPPORTING-CONTEXTS" NULL 0))
   ...
   (CL:SETQ SGT-JUSTIFICATIONS-PL-KERNEL-KB-SUBSET-OF
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SUBSET-OF"
     (GET-STELLA-MODULE "/PL-KERNEL-KB" CL:T) 1))
   (CL:SETQ SGT-JUSTIFICATIONS-PL-KERNEL-KB-EQUIVALENT
...
(CL:DEFUN HELP-STARTUP-JUSTIFICATIONS3 ()
  (CL:PROGN
   (DEFINE-INLINE-METHOD-CODE SGT-JUSTIFICATIONS-LOGIC-JUSTIFICATION
    SYM-JUSTIFICATIONS-LOGIC-INFERENCE-STRATEGY
    (WRAP-METHOD-CODE (CL:FUNCTION INFERENCE-STRATEGY)) NULL)
   (DEFINE-INLINE-METHOD-CODE SGT-JUSTIFICATIONS-LOGIC-JUSTIFICATION
    SYM-JUSTIFICATIONS-LOGIC-INFERENCE-DIRECTION
    (WRAP-METHOD-CODE (CL:FUNCTION INFERENCE-DIRECTION)) NULL)
...
all new justification objects, but structure-shares other information such
as propositions and substitutions.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (WRAP-METHOD-CODE (CL:FUNCTION SHALLOW-COPY)) NULL)
...
pattern nodes, and :BRIEF additionally suppresses AND-introduction nodes.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-INLINE-METHOD-CODE
    SGT-JUSTIFICATIONS-LOGIC-FORWARD-JUSTIFICATION
    SYM-JUSTIFICATIONS-STELLA-DELETED?
    (WRAP-METHOD-CODE (CL:FUNCTION DELETED?)) NULL)
   (DEFINE-INLINE-METHOD-CODE
    SGT-JUSTIFICATIONS-LOGIC-FORWARD-JUSTIFICATION
    SYM-JUSTIFICATIONS-LOGIC-SUPPORTED?
    (WRAP-METHOD-CODE (CL:FUNCTION SUPPORTED?)) NULL)
   (DEFINE-EXTERNAL-SLOT-FROM-STRINGIFIED-SOURCE
   ...
   (DEFINE-FUNCTION-OBJECT "HAS-FORWARD-JUSTIFICATIONS?"
    "(DEFUN (HAS-FORWARD-JUSTIFICATIONS? BOOLEAN) ((PROPOSITION PROPOSITION)) :DOCUMENTATION \"Return TRUE if `proposition' has any viable forward justifications.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NON-EMPTY? (REMOVE-DELETED-MEMBERS (FORWARD-JUSTIFICATIONS PROPOSITION)))))"
    (CL:FUNCTION HAS-FORWARD-JUSTIFICATIONS?) NULL)
   (DEFINE-FUNCTION-OBJECT "HAS-SUPPORTED-FORWARD-JUSTIFICATIONS?"
    "(DEFUN (HAS-SUPPORTED-FORWARD-JUSTIFICATIONS? BOOLEAN) ((PROPOSITION PROPOSITION)) :DOCUMENTATION \"Return TRUE if `proposition' has any forward justifications supported by the current context.\" :PUBLIC? TRUE)"
    (CL:FUNCTION HAS-SUPPORTED-FORWARD-JUSTIFICATIONS?) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-FORWARD-JUSTIFICATIONS"
    "(DEFUN (GET-FORWARD-JUSTIFICATIONS (LIST OF FORWARD-JUSTIFICATION)) ((PROPOSITION PROPOSITION)) :DOCUMENTATION \"Return `proposition's forward justifications.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (REMOVE-DELETED-MEMBERS (FORWARD-JUSTIFICATIONS PROPOSITION))))"
    (CL:FUNCTION GET-FORWARD-JUSTIFICATIONS) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-SUPPORTED-FORWARD-JUSTIFICATIONS"
    "(DEFUN (GET-SUPPORTED-FORWARD-JUSTIFICATIONS (LIST OF FORWARD-JUSTIFICATION)) ((PROPOSITION PROPOSITION)) :DOCUMENTATION \"Return all of `proposition's forward justifications supported in the current context.\" :PUBLIC? TRUE)"
    (CL:FUNCTION GET-SUPPORTED-FORWARD-JUSTIFICATIONS) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION"
    "(DEFUN (GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION FORWARD-JUSTIFICATION) ((PROPOSITION PROPOSITION)) :DOCUMENTATION \"Return one of `proposition's forward justifications supported in the current context.\" :PUBLIC? TRUE)"
    (CL:FUNCTION GET-ONE-SUPPORTED-FORWARD-JUSTIFICATION) NULL)
   (DEFINE-FUNCTION-OBJECT "ADD-FORWARD-JUSTIFICATION"
    "(DEFUN ADD-FORWARD-JUSTIFICATION ((PROPOSITION PROPOSITION) (JUSTIFICATION JUSTIFICATION)))"
    (CL:FUNCTION ADD-FORWARD-JUSTIFICATION) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CREATE-FORWARD-RULE-JUSTIFICATION"
    "(DEFUN (CREATE-FORWARD-RULE-JUSTIFICATION FORWARD-JUSTIFICATION) ((ANTECEDENTS (CONS OF PROPOSITION)) (FORWARDRULE PROPOSITION) (ARGUMENTS ARGUMENTS-VECTOR) (CONSEQUENTPROPOSITION PROPOSITION) (BC-JUSTIFICATION JUSTIFICATION)))"
    (CL:FUNCTION CREATE-FORWARD-RULE-JUSTIFICATION) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "RECORD-FORWARD-RULE-JUSTIFICATION"
    "(DEFUN RECORD-FORWARD-RULE-JUSTIFICATION ((ANTECEDENTS (CONS OF PROPOSITION)) (FORWARDRULE PROPOSITION) (ARGUMENTS ARGUMENTS-VECTOR) (CONSEQUENTPROPOSITION PROPOSITION) (BC-JUSTIFICATION JUSTIFICATION)))"
    (CL:FUNCTION RECORD-FORWARD-RULE-JUSTIFICATION) NULL)
   (DEFINE-FUNCTION-OBJECT "CREATE-SUBSET-OF-JUSTIFICATION"
    "(DEFUN (CREATE-SUBSET-OF-JUSTIFICATION JUSTIFICATION) ((SUB OBJECT) (SUPER OBJECT)))"
    (CL:FUNCTION CREATE-SUBSET-OF-JUSTIFICATION) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS"
    "(DEFUN (GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS (CONS OF PROPOSITION)) ((X SKOLEM) (Y OBJECT)))"
    (CL:FUNCTION GET-SUPPORTING-EQUIVALENCE-PROPOSITIONS) NULL)
   (DEFINE-FUNCTION-OBJECT "FIND-EQUIVALENCE-PROPOSITION-PATH"
    "(DEFUN (FIND-EQUIVALENCE-PROPOSITION-PATH (CONS OF PROPOSITION)) ((EQUIVPROPS (CONS OF PROPOSITION)) (X SKOLEM) (Y OBJECT)))"
    (CL:FUNCTION FIND-EQUIVALENCE-PROPOSITION-PATH) NULL)
   (DEFINE-FUNCTION-OBJECT "CREATE-EQUIVALENCE-JUSTIFICATIONS"
    "(DEFUN (CREATE-EQUIVALENCE-JUSTIFICATIONS (CONS OF JUSTIFICATION)) ((X OBJECT) (Y OBJECT)))"
    (CL:FUNCTION CREATE-EQUIVALENCE-JUSTIFICATIONS) NULL)
   (DEFINE-FUNCTION-OBJECT "CREATE-IMPLICIT-INFERENCE-JUSTIFICATION"
    "(DEFUN (CREATE-IMPLICIT-INFERENCE-JUSTIFICATION JUSTIFICATION) ((INFERREDPROP PROPOSITION) (SUPPORTPROP PROPOSITION)))"
    (CL:FUNCTION CREATE-IMPLICIT-INFERENCE-JUSTIFICATION) NULL)
   (DEFINE-INLINE-METHOD-CODE
    SGT-JUSTIFICATIONS-LOGIC-CLASH-JUSTIFICATION
    SYM-JUSTIFICATIONS-LOGIC-INFERENCE-DIRECTION
    (WRAP-METHOD-CODE (CL:FUNCTION INFERENCE-DIRECTION)) NULL)
    ...
    (CL:LET*
     ((CLASS
       (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "CLASH-JUSTIFICATION"
        "(DEFCLASS CLASH-JUSTIFICATION (FORWARD-JUSTIFICATION) :SLOTS ((DIRECTION :TYPE KEYWORD :INITIALLY :FORWARD :DOCUMENTATION \"The inference direction for this inference.\")) :METHODS ((INFERENCE-DIRECTION ((SELF CLASH-JUSTIFICATION)) :TYPE KEYWORD (RETURN (DIRECTION SELF)))))")))
     (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS)

...
(CL:DEFVAR SYM-GOAL-CACHES-LOGIC-MATCH-SCORE NULL)
...
(CL:DEFVAR SGT-GOAL-CACHES-LOGIC-PROPOSITION NULL)
...
(CL:DEFUN ACCESS-ATOMIC-GOAL-CACHE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-GOAL-CACHES-LOGIC-MATCH-SCORE)
    (CL:IF SETVALUE?
...
(CL:DEFUN CACHE-GOAL (FRAME SUCCESS? KEEPFRAME? CLOCKTICKS)
  ...
  (CL:WHEN (CL:EQ (%STATE FRAME) KWD-GOAL-CACHES-ATOMIC-GOAL)
   ...
   (CL:LET* ((BINDING NULL) (ITER-000 (%GOAL-BINDINGS FRAME)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (CL:EQ BINDING NULL))
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE BINDING)))
       (CL:COND
          ...
          (CL:WHEN
           (CL:NOT
            (CL:EQ
             ...
             NULL))
           (CL:RETURN-FROM CACHE-GOAL))))
        ((SUBTYPE-OF? TEST-VALUE-000 SGT-GOAL-CACHES-LOGIC-PROPOSITION)
         (CL:PROGN (CL:RETURN-FROM CACHE-GOAL)))
        (CL:T))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   ...
   (CL:WHEN
    ...
    (CL:LET* ((TEST-VALUE-001 (%KIND (%PROPOSITION FRAME))))
     (CL:COND
      ((CL:OR (CL:EQ TEST-VALUE-001 KWD-GOAL-CACHES-ISA)
        (CL:EQ TEST-VALUE-001 KWD-GOAL-CACHES-PREDICATE)
        (CL:EQ TEST-VALUE-001 KWD-GOAL-CACHES-FUNCTION))
       (CL:WHEN
        (MEMBER?
         (GET-QUOTED-TREE
...
(CL:DEFUN HELP-STARTUP-GOAL-CACHES1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-GOAL-CACHES-LOGIC-MATCH-SCORE
   ...
   (CL:SETQ SGT-GOAL-CACHES-LOGIC-PROPOSITION
...
for some named description\" :ALLOCATION :DYNAMIC)")
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *CACHE-SUCCEEDED-GOALS?* BOOLEAN TRUE)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *CACHE-FAILED-GOALS?* BOOLEAN TRUE)" NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT

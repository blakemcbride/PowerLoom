...
(CL:DEFVAR SGT-SPECIALIZE-PL-KERNEL-KB-HOLDS NULL)
...
(CL:DECLAIM
  ...
  *CONTEXT* *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE TRUE-WRAPPER
  FALSE-WRAPPER *CYC-KLUDGES-ENABLED?* *QUERYITERATOR* NULL-INTEGER
...
(CL:DEFUN NEXT-EQUIVALENT-SELECTION-PATTERN (SELF)
  (CL:LET*
   ((PATTERN (%SELECTION-PATTERN SELF))
    (KEYWITHEQUIVALENTS (FIND-SELECTION-KEY-WITH-EQUIVALENTS PATTERN))
    (EQUIVALENTSKOLEMS (%EQUIVALENTS-STACK SELF)) (NEWSKOLEM NULL))
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ KEYWITHEQUIVALENTS NULL))
     (CL:EQ EQUIVALENTSKOLEMS NULL))
    (CL:SETQ EQUIVALENTSKOLEMS
     (GET-ALL-EQUIVALENT-VALUES KEYWITHEQUIVALENTS)))
   (CL:WHEN
    (CL:OR (CL:EQ EQUIVALENTSKOLEMS NULL)
     (CL:EQ EQUIVALENTSKOLEMS NIL))
    (CL:RETURN-FROM NEXT-EQUIVALENT-SELECTION-PATTERN NULL))
   (CL:SETQ NEWSKOLEM (%%VALUE EQUIVALENTSKOLEMS))
   (CL:SETF (%EQUIVALENTS-STACK SELF) (%%REST EQUIVALENTSKOLEMS))
...
(CL:DEFUN EMPTY-PROPOSITIONS-INDEX? (INDEX PRIMARYKEY SPECIALIZE?)
  (CL:AND (CL:NOT SPECIALIZE?)
   ...
   (CL:NOT (HAS-EQUIVALENT-VALUES? PRIMARYKEY))))
...
;;; (DEFUN (RELATION-AND-ARGUMENTS-UNIFY? BOOLEAN) ...)

(CL:DEFUN RELATION-AND-ARGUMENTS-UNIFY? (SUBPROPOSITION REFERENCEPROPOSITION DONTUNIFYRELATION?)
  (CL:LET*
   ((PATTERNRECORD (%CURRENT-PATTERN-RECORD *QUERYITERATOR*))
    (UBSTACKOFFSET (%TOP-UNBINDING-STACK-OFFSET PATTERNRECORD))
    (REFSTART 0) (REFARGS (%ARGUMENTS REFERENCEPROPOSITION))
    (NUMREFARGS (LENGTH REFARGS)) (SUBSTART 0)
    (SUBARGS (%ARGUMENTS SUBPROPOSITION)) (NUMSUBARGS (LENGTH SUBARGS))
    (SUCCESS? CL:T))
   (CL:DECLARE
    (CL:TYPE CL:FIXNUM UBSTACKOFFSET REFSTART NUMREFARGS SUBSTART
     NUMSUBARGS))
   (CL:WHEN (CL:NOT DONTUNIFYRELATION?)
    (CL:WHEN
     (CL:EQ (%OPERATOR REFERENCEPROPOSITION)
      SGT-SPECIALIZE-PL-KERNEL-KB-HOLDS)
     (CL:SETQ REFSTART (CL:1+ REFSTART)))
    (CL:WHEN
     (CL:EQ (%OPERATOR SUBPROPOSITION)
      SGT-SPECIALIZE-PL-KERNEL-KB-HOLDS)
     (CL:SETQ SUBSTART (CL:1+ SUBSTART)))
    (CL:LET*
     ((SUBOPERATOR
       (CL:IF (CL:> SUBSTART 0)
        (ARGUMENT-BOUND-TO
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SUBARGS)) 0))
        (%SURROGATE-VALUE (%OPERATOR SUBPROPOSITION)))))
     (CL:COND ((CL:EQ SUBOPERATOR NULL) (CL:SETQ SUCCESS? CL:NIL))
      ((CL:> REFSTART 0)
       (CL:SETQ SUCCESS?
        (BIND-ARGUMENT-TO-VALUE?
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY REFARGS)) 0)
         SUBOPERATOR CL:NIL)))
      (CL:T
       (CL:SETQ SUCCESS?
        (EQL? (%SURROGATE-VALUE (%OPERATOR REFERENCEPROPOSITION))
         SUBOPERATOR))))))
   (CL:WHEN SUCCESS?
    (CL:LET* ((TEST-VALUE-000 CL:NIL))
     (CL:LET* ((ALWAYS?-000 CL:T))
      (CL:LET*
       ((SUPI NULL-INTEGER) (ITER-000 REFSTART)
        (UPPER-BOUND-000 (CL:1- NUMREFARGS)) (SUBI NULL-INTEGER)
        (ITER-001 SUBSTART) (UPPER-BOUND-001 (CL:1- NUMSUBARGS)))
       (CL:DECLARE
        (CL:TYPE CL:FIXNUM SUPI ITER-000 UPPER-BOUND-000 SUBI ITER-001
         UPPER-BOUND-001))
       (CL:LOOP WHILE
        (CL:AND (CL:<= ITER-000 UPPER-BOUND-000)
         (CL:<= ITER-001 UPPER-BOUND-001))
        DO (CL:SETQ SUPI ITER-000) (CL:SETQ SUBI ITER-001)
        (CL:WHEN
         (CL:NOT
          (CL:AND
           (CL:NOT
            (NULL-INSTANCE?
             (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SUBARGS))
              SUBI)))
           (BIND-ARGUMENT-TO-VALUE?
            (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY REFARGS))
             SUPI)
            (VALUE-OF
             (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SUBARGS))
              SUBI))
     ...
     (CL:WHEN TEST-VALUE-000
      (CL:SETQ TEST-VALUE-000
       (CL:= (CL:- NUMREFARGS REFSTART) (CL:- NUMSUBARGS SUBSTART))))
...
has to be also for unification to succeed."
  (RELATION-AND-ARGUMENTS-UNIFY? SUBPROPOSITION REFERENCEPROPOSITION
   CL:T))
...
;;; (DEFUN (TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR (ITERATOR OF PROPOSITION)) ...)

(CL:DEFUN TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR (SELF)
  (CL:WHEN (CL:EQ SELF EMPTY-PROPOSITIONS-ITERATOR)
   (CL:RETURN-FROM TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR SELF))
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000
      SGT-SPECIALIZE-LOGIC-SPECIALIZING-PROPOSITIONS-ITERATOR)
     (CL:PROGN (%PROPOSITION-CURSOR SELF)))
    ((SUBTYPE-OF? TEST-VALUE-000
      SGT-SPECIALIZE-LOGIC-TRUE-PROPOSITIONS-ITERATOR)
     (CL:PROGN (%PROPOSITION-CURSOR SELF)))
    ((SUBTYPE-OF? TEST-VALUE-000
      SGT-SPECIALIZE-LOGIC-DESCRIPTION-EXTENSION-ITERATOR)
     (CL:PROGN (%EXTENSION-ITERATOR SELF)))
    (CL:T EMPTY-PROPOSITIONS-ITERATOR))))
...
(CL:DEFUN HELP-STARTUP-SPECIALIZE1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-SPECIALIZE-PL-KERNEL-KB-HOLDS
...
(CL:DEFUN HELP-STARTUP-SPECIALIZE3 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "RELATION-AND-ARGUMENTS-UNIFY?"
    "(DEFUN (RELATION-AND-ARGUMENTS-UNIFY? BOOLEAN) ((SUBPROPOSITION PROPOSITION) (REFERENCEPROPOSITION PROPOSITION) (DONTUNIFYRELATION? BOOLEAN)))"
    (CL:FUNCTION RELATION-AND-ARGUMENTS-UNIFY?) NULL)
   (DEFINE-FUNCTION-OBJECT "ARGUMENTS-UNIFY-WITH-ARGUMENTS?"
    "(DEFUN (ARGUMENTS-UNIFY-WITH-ARGUMENTS? BOOLEAN) ((SUBPROPOSITION PROPOSITION) (REFERENCEPROPOSITION PROPOSITION)) :DOCUMENTATION \"Return TRUE if the arguments in proposition 'subProposition' unify with
those in 'referenceProposition'.  If one of the propositions is a HOLDS, the other one
has to be also for unification to succeed.\" :GLOBALLY-INLINE? TRUE (RETURN (RELATION-AND-ARGUMENTS-UNIFY? SUBPROPOSITION REFERENCEPROPOSITION TRUE)))"
   ...
   (DEFINE-FUNCTION-OBJECT "TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR"
    "(DEFUN (TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR (ITERATOR OF PROPOSITION)) ((SELF (ITERATOR OF PROPOSITION))))"
    (CL:FUNCTION TRUE-TO-CONCEIVED-PROPOSITIONS-ITERATOR) NULL)

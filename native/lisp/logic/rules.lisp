...
(CL:DEFVAR SYM-RULES-STELLA-AND NULL)
(CL:DEFVAR KWD-RULES-CONCEIVE NULL)
(CL:DEFVAR SYM-RULES-PL-KERNEL-KB-SKOLEM-FN NULL)
(CL:DEFVAR SYM-RULES-LOGIC-<<= NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* TRUE-TRUTH-VALUE
  FALSE-TRUTH-VALUE DEFAULT-TRUE-TRUTH-VALUE DEFAULT-FALSE-TRUTH-VALUE
  UNKNOWN-TRUTH-VALUE *REVERSEPOLARITY?* NIL-LIST
    ...
    (CL:WHEN
     (CL:AND
      (%WRAPPER-VALUE
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FORALLPROP)
        SYM-RULES-LOGIC-BACKWARD-ONLY? FALSE-WRAPPER))
      (CL:EQ GOALDESCRIPTION NULL)
      (EMPTY? (FIND-MATCHABLE-GOALS FORALLPROP KWD-RULES-HEAD)))
     (CL:SETQ CREATEDSATELLITES? CL:T) (SKOLEMIZE-RULE FORALLPROP))
    (CL:WHEN
     (CL:AND
      (CL:NOT
       (%WRAPPER-VALUE
    ...
    (CL:AND CREATEDSATELLITES?
     (CL:NOT
      (CL:EQ
       ...
       NULL)))
    (CL:LET*
     ((*CONTEXT* *CONTEXT*) (*MODULE* (%BASE-MODULE *CONTEXT*)))
     (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*))
...
;;; (DEFUN SKOLEMIZE-RULE ...)

(CL:DEFUN SKOLEMIZE-RULE (RULE)
  (CL:WHEN
   (CL:NOT
    (CL:AND (CL:EQ (%KIND RULE) KWD-RULES-FORALL)
     (%WRAPPER-VALUE
     ...
     (CL:EQ
      ...
      KWD-RULES-EXISTS)))
   (CL:RETURN-FROM SKOLEMIZE-RULE))
  (CL:LET*
   ((ARROW NULL) (ANTECEDENT NIL) (CONSEQUENTS NIL) (ANTVARS NIL)
    (CQVARS NIL) (RULETREE NIL) (MODULE (HOME-MODULE RULE))
    (SKOLEMFNID
     (INTERN-SYMBOL-IN-MODULE (YIELD-UNIQUE-GENSYM-NAME "skfn" MODULE)
      MODULE CL:T))
    (SATELLITE NULL))
   (CL:SETQ RULETREE (GENERATE-TERM RULE))
   (CL:SETQ ARROW (CONS-TREE-NTH RULETREE 2 0))
   (CL:SETQ ANTVARS (%%VALUE (%%REST RULETREE)))
   (CL:COND
    ((CL:EQL
      (CL:LET ((SELF (%SYMBOL-NAME ARROW)) (POSITION 0))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
        (CL:THE CL:FIXNUM POSITION)))
      #\<)
     (CL:SETQ ANTECEDENT (CONS-TREE-NTH RULETREE 2 2))
     (CL:SETQ CQVARS (CONS-TREE-NTH RULETREE 2 1 1))
     (CL:SETQ CONSEQUENTS (CONS-TREE-NTH RULETREE 2 1 2)))
    (CL:T (CL:SETQ ANTECEDENT (CONS-TREE-NTH RULETREE 2 1))
     (CL:SETQ CQVARS (CONS-TREE-NTH RULETREE 2 2 1))
     (CL:SETQ CONSEQUENTS (CONS-TREE-NTH RULETREE 2 2 2))))
   (CL:IF (CL:EQ (%%VALUE CONSEQUENTS) SYM-RULES-STELLA-AND)
    (CL:SETQ CONSEQUENTS (%%REST CONSEQUENTS))
    (CL:SETQ CONSEQUENTS (CONS CONSEQUENTS NIL)))
   (CL:LET* ((GOAL NULL) (ITER-000 CONSEQUENTS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ GOAL (%%VALUE ITER-000))
     (CL:SETQ SATELLITE
      (%%VALUE
       (UPDATE-PROPOSITION
        (SKOLEMIZE-RULE-FOR-GOAL GOAL ANTECEDENT ANTVARS CQVARS
         SKOLEMFNID)
        KWD-RULES-CONCEIVE)))
     (CL:WHEN (EMPTY? (SATELLITE-PROPOSITIONS RULE))
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS RULE)
       SYM-RULES-LOGIC-SATELLITE-PROPOSITIONS (NEW-LIST) NULL))
     (INSERT-NEW (SATELLITE-PROPOSITIONS RULE) SATELLITE)
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SATELLITE)
      SYM-RULES-LOGIC-MASTER-PROPOSITION RULE NULL)
     (CL:SETQ ITER-000 (%%REST ITER-000))))))

;;; (DEFUN (SKOLEMIZE-RULE-FOR-GOAL CONS) ...)

(CL:DEFUN SKOLEMIZE-RULE-FOR-GOAL (GOAL ANTECEDENT FORALLVARS EXISTSVARS SKOLEMFNID)
  (CL:LET* ((SKOLEMCLAUSES NIL) (SKOLEMIZEDRULE NIL))
   (CL:LET* ((VAR NULL) (ITER-000 EXISTSVARS) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ VAR (%%VALUE ITER-000))
     (CL:WHEN (SEARCH-CONS-TREE? GOAL VAR)
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN
        (CL:SETQ COLLECT-000
         (CONS
          (LIST* SYM-RULES-PL-KERNEL-KB-SKOLEM-FN SKOLEMFNID
           (CONS (WRAP-STRING (%SYMBOL-NAME VAR))
            (CONCATENATE (COPY-CONS-LIST FORALLVARS) (CONS VAR NIL))))
          NIL))
        (CL:IF (CL:EQ SKOLEMCLAUSES NIL)
         (CL:SETQ SKOLEMCLAUSES COLLECT-000)
         (ADD-CONS-TO-END-OF-CONS-LIST SKOLEMCLAUSES COLLECT-000)))
       (CL:PROGN
     ...
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:IF (CL:EQ SKOLEMCLAUSES NIL)
    (CL:SETQ SKOLEMIZEDRULE
     (LIST* SYM-RULES-LOGIC-<<= GOAL (CONS ANTECEDENT NIL)))
    (CL:SETQ SKOLEMIZEDRULE
     (LIST* SYM-RULES-LOGIC-<<= GOAL
      (CONS
       (LIST* SYM-RULES-STELLA-AND ANTECEDENT
        (CONCATENATE SKOLEMCLAUSES NIL))
       NIL))))
   (COPY-CONS-TREE SKOLEMIZEDRULE)))
...
;;; (DEFUN (PROPOSITION-HAS-STRENGTH? BOOLEAN) ...)

(CL:DEFUN PROPOSITION-HAS-STRENGTH? (SELF MINIMALSTRENGTH)
  "Return true if the truth-value of `self' satisfies `minimalStrength'."
  (CL:COND
   ((CL:EQ MINIMALSTRENGTH NULL)
    (CL:RETURN-FROM PROPOSITION-HAS-STRENGTH? CL:T))
   ((CL:OR (CL:EQ MINIMALSTRENGTH TRUE-TRUTH-VALUE)
     (CL:EQ MINIMALSTRENGTH FALSE-TRUTH-VALUE))
    (CL:WHEN
     (CL:NOT
      (STRICT-TRUTH-VALUE?
       (ACCESS-IN-CONTEXT (%TRUTH-VALUE SELF) (%HOME-CONTEXT SELF)
        CL:NIL)))
     (CL:RETURN-FROM PROPOSITION-HAS-STRENGTH? CL:NIL)))
   ((CL:OR (CL:EQ MINIMALSTRENGTH DEFAULT-TRUE-TRUTH-VALUE)
     (CL:EQ MINIMALSTRENGTH DEFAULT-FALSE-TRUTH-VALUE))
    (CL:WHEN
     (UNKNOWN-TRUTH-VALUE?
      (ACCESS-IN-CONTEXT (%TRUTH-VALUE SELF) (%HOME-CONTEXT SELF)
       CL:NIL))
...
(CL:DEFUN STARTUP-RULES ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "SKOLEMIZE-RULE"
     "(DEFUN SKOLEMIZE-RULE ((RULE PROPOSITION)))"
     (CL:FUNCTION SKOLEMIZE-RULE) NULL)
    (DEFINE-FUNCTION-OBJECT "SKOLEMIZE-RULE-FOR-GOAL"
     "(DEFUN (SKOLEMIZE-RULE-FOR-GOAL CONS) ((GOAL CONS) (ANTECEDENT CONS) (FORALLVARS (CONS OF SYMBOL)) (EXISTSVARS (CONS OF SYMBOL)) (SKOLEMFNID SYMBOL)))"
     (CL:FUNCTION SKOLEMIZE-RULE-FOR-GOAL) NULL)
...
a user wants to verify the behavior of different sets of rules.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)"
    ...
    (DEFINE-FUNCTION-OBJECT "PROPOSITION-HAS-STRENGTH?"
     "(DEFUN (PROPOSITION-HAS-STRENGTH? BOOLEAN) ((SELF PROPOSITION) (MINIMALSTRENGTH TRUTH-VALUE)) :DOCUMENTATION \"Return true if the truth-value of `self' satisfies `minimalStrength'.\" :PUBLIC? TRUE)"
     (CL:FUNCTION PROPOSITION-HAS-STRENGTH?) NULL)

...
(CL:DEFVAR SGT-BACKLINKS-PL-KERNEL-KB-EQUIVALENT NULL)
...
(CL:DEFVAR SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003 NULL)
(CL:DEFVAR SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004 NULL)
...
(CL:DECLAIM
  ...
  *CONTEXT* *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE
  *DATA-FINDER-KLUDGES-ENABLED?* TRUE-WRAPPER *ALL-OBJECT-STORES*
  NIL-LIST NULL-INTEGER STANDARD-OUTPUT NIL-NON-PAGING-INDEX
  *POWERLOOM-EXECUTION-MODE* STANDARD-WARNING EOL FALSE-WRAPPER NIL
  *MODULE*))
   ...
   (CL:COND
    ...
    (CL:T
     (CL:LET* ((SURROGATE (%OPERATOR PROPOSITION)) (BUCKET NULL))
      ...
      (CL:WHEN (CL:EQ SURROGATE SGT-BACKLINKS-PL-KERNEL-KB-EQUIVALENT)
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
         (CL:SETQ ARG
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
           INDEX-000))
         (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? ARG)
          (ADD-DEPENDENT-PROPOSITION-TO-SPECIALIZED-INDEX INDEX
           (%DEFINING-PROPOSITION ARG) ARGUMENT))
...
(CL:DEFUN REMOVE-DEPENDENT-PROPOSITION-LINK (SELF PROPOSITION)
  ...
  (CL:WHEN
   (CL:EQ (%OPERATOR PROPOSITION)
    SGT-BACKLINKS-PL-KERNEL-KB-EQUIVALENT)
   (CL:LET*
    ((ARG NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION)) (INDEX-000 0)
     (LENGTH-000 (LENGTH VECTOR-000)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     (CL:SETQ ARG
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
       INDEX-000))
     (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? ARG)
      (REMOVE-DEPENDENT-PROPOSITION-LINK SELF
       (%DEFINING-PROPOSITION ARG)))
     (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))))
  ...
  (CL:LET* ((CANDIDATEKEYS (%%REST (%%REST PATTERN))))
   ...
   (CL:LET* ((KEY NULL) (ITER-000 CANDIDATEKEYS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (HAS-EQUIVALENT-VALUES? KEY)
...
(CL:DEFUN HELP-SELECT-RELATION-PROPOSITIONS (PATTERN)
  (CL:LET*
   ((ARGUMENTS (%%REST (%%REST PATTERN)))
    ...
    (ANCHOR NULL) (FUNCTION-SKOLEM NULL))
    ...
    (CL:LET* ((VALUE-000 NULL))
     (CL:LET* ((KEY NULL) (ITER-000 KEYS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:SETQ KEY (%%VALUE ITER-000))
       (CL:WHEN
        (CL:AND (CL:NOT (CL:EQ KEY NULL))
         (FUNCTION-OUTPUT-SKOLEM? KEY))
        (CL:SETQ VALUE-000 KEY) (CL:RETURN))
       (CL:SETQ ITER-000 (%%REST ITER-000))))
     (CL:SETQ ANCHOR VALUE-000)))
   (CL:WHEN (CL:EQ ANCHOR NULL)
    (CL:LET* ((VALUE-001 NULL))
     (CL:LET* ((KEY NULL) (ITER-001 KEYS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:SETQ KEY (%%VALUE ITER-001))
       (CL:WHEN
        (CL:AND (CL:NOT (CL:EQ KEY NULL))
         (INSTANCE-HAS-BACKLINKS? KEY))
        (CL:SETQ VALUE-001 KEY) (CL:RETURN))
       (CL:SETQ ITER-001 (%%REST ITER-001))))
     (CL:SETQ ANCHOR VALUE-001)))
   (CL:WHEN (CL:EQ ANCHOR NULL)
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:EQ SEQUENCE NIL)
     (CL:RETURN-FROM HELP-SELECT-RELATION-PROPOSITIONS
      NIL-NON-PAGING-INDEX))
    (CL:LET* ((PROP NULL) (ITER-002 SEQUENCE))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
      (CL:SETQ PROP (%%VALUE ITER-002))
      (CL:TAGBODY
       (CL:WHEN (CL:NOT (EQUAL? RELATION (%OPERATOR PROP)))
        (CL:WHEN *DATA-FINDER-KLUDGES-ENABLED?* (CL:GO :CONTINUE))
        (CL:COND
       ...
       (CL:WHEN (CL:EQ ALLKEYVALUES NULL) (CL:SETQ ALLKEYVALUES NIL)
        (CL:WHEN (CL:NOT (CL:EQ (%%REST KEYS) NIL))
         (CL:LET* ((KEY NULL) (ITER-003 KEYS) (COLLECT-000 NULL))
        ...
        (CL:LET* ((ARGUMENTS (%ARGUMENTS PROP)))
         (CL:LET* ((TEST-VALUE-000 CL:NIL))
          ...
          (CL:WHEN TEST-VALUE-000
           (CL:LET* ((ALWAYS?-000 CL:T))
            (CL:LET*
             ((KEY NULL) (ITER-004 ALLKEYVALUES) (ARG NULL)
    ...
    (CL:IF (CL:EQ FILTEREDSEQUENCE NIL) NIL-NON-PAGING-INDEX
     (CL:LET* ((SELF-001 (NEW-NON-PAGING-INDEX)))
      ...
      (CL:LET* ((VALUE-002 SELF-001)) VALUE-002))))))
...
(CL:DEFUN SELECT-RELATION-PROPOSITIONS (PATTERN)
  (CL:LET*
   ((ARGUMENTS (%%REST (%%REST PATTERN)))
    (RELATION (%%VALUE ARGUMENTS)) (KEY1 NULL) (KEY2 NULL) (KEY3 NULL)
    (KEY4 NULL))
   ...
   (CL:SETQ KEY3 (%%VALUE ARGUMENTS))
   (CL:SETQ ARGUMENTS (%%REST ARGUMENTS))
   (CL:WHEN (CL:EQ ARGUMENTS NIL)
    (CL:LET*
     ((MEMO-TABLE-003 NULL) (MEMOIZED-ENTRY-003 NULL)
      (MEMOIZED-VALUE-003 NULL))
     (CL:WHEN *MEMOIZATION-ENABLED?*
      (CL:SETQ MEMO-TABLE-003
       (%SURROGATE-VALUE
        SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003))
      (CL:WHEN (CL:EQ MEMO-TABLE-003 NULL)
       (INITIALIZE-MEMOIZATION-TABLE
        SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003
        "(:MAX-VALUES 1000 :TIMESTAMPS (:KB-UPDATE))")
       (CL:SETQ MEMO-TABLE-003
        (%SURROGATE-VALUE
         SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003)))
      (CL:SETQ MEMOIZED-ENTRY-003
       (LOOKUP-MRU-MEMOIZED-VALUEN MEMO-TABLE-003
        (CONS-LIST RELATION KEY1 KEY2 KEY3 *CONTEXT*) 16))
      (CL:SETQ MEMOIZED-VALUE-003 (%%VALUE MEMOIZED-ENTRY-003)))
     (CL:COND
      ((CL:NOT (CL:EQ MEMOIZED-VALUE-003 NULL))
       (CL:WHEN (CL:EQ MEMOIZED-VALUE-003 MEMOIZED-NULL-VALUE)
        (CL:SETQ MEMOIZED-VALUE-003 NULL)))
      (CL:T
       (CL:SETQ MEMOIZED-VALUE-003
        (HELP-SELECT-RELATION-PROPOSITIONS PATTERN))
       (CL:WHEN *MEMOIZATION-ENABLED?*
        (CL:SETF (%%VALUE MEMOIZED-ENTRY-003)
         (CL:IF (CL:EQ MEMOIZED-VALUE-003 NULL) MEMOIZED-NULL-VALUE
          MEMOIZED-VALUE-003)))))
     (CL:LET* ((VALUE-003 MEMOIZED-VALUE-003))
      (CL:RETURN-FROM SELECT-RELATION-PROPOSITIONS VALUE-003))))
   (CL:SETQ KEY4 (%%VALUE ARGUMENTS))
   (CL:SETQ ARGUMENTS (%%REST ARGUMENTS))
   (CL:WHEN (CL:EQ ARGUMENTS NIL)
    (CL:LET*
     ((MEMO-TABLE-004 NULL) (MEMOIZED-ENTRY-004 NULL)
      (MEMOIZED-VALUE-004 NULL))
     (CL:WHEN *MEMOIZATION-ENABLED?*
      (CL:SETQ MEMO-TABLE-004
       (%SURROGATE-VALUE
        SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004))
      (CL:WHEN (CL:EQ MEMO-TABLE-004 NULL)
       (INITIALIZE-MEMOIZATION-TABLE
        SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004
        "(:MAX-VALUES 1000 :TIMESTAMPS (:KB-UPDATE))")
       (CL:SETQ MEMO-TABLE-004
        (%SURROGATE-VALUE
         SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004)))
      (CL:SETQ MEMOIZED-ENTRY-004
       (LOOKUP-MRU-MEMOIZED-VALUEN MEMO-TABLE-004
        (CONS-LIST RELATION KEY1 KEY2 KEY3 KEY4 *CONTEXT*) 32))
      (CL:SETQ MEMOIZED-VALUE-004 (%%VALUE MEMOIZED-ENTRY-004)))
     (CL:COND
      ((CL:NOT (CL:EQ MEMOIZED-VALUE-004 NULL))
       (CL:WHEN (CL:EQ MEMOIZED-VALUE-004 MEMOIZED-NULL-VALUE)
        (CL:SETQ MEMOIZED-VALUE-004 NULL)))
      (CL:T
       (CL:SETQ MEMOIZED-VALUE-004
        (HELP-SELECT-RELATION-PROPOSITIONS PATTERN))
       (CL:WHEN *MEMOIZATION-ENABLED?*
        (CL:SETF (%%VALUE MEMOIZED-ENTRY-004)
         (CL:IF (CL:EQ MEMOIZED-VALUE-004 NULL) MEMOIZED-NULL-VALUE
          MEMOIZED-VALUE-004)))))
     (CL:LET* ((VALUE-004 MEMOIZED-VALUE-004))
      (CL:RETURN-FROM SELECT-RELATION-PROPOSITIONS VALUE-004))))
   (HELP-SELECT-RELATION-PROPOSITIONS PATTERN)))
...
(CL:DEFUN HELP-STARTUP-BACKLINKS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-BACKLINKS-PL-KERNEL-KB-EQUIVALENT
   ...
   (CL:SETQ
    SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003
    (INTERN-RIGID-SYMBOL-WRT-MODULE
     "F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-003" NULL 1))
   (CL:SETQ
    SGT-BACKLINKS-LOGIC-F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004
    (INTERN-RIGID-SYMBOL-WRT-MODULE
     "F-SELECT-RELATION-PROPOSITIONS-MEMO-TABLE-004" NULL 1))
...
first class logic objects.\" :PUBLIC? TRUE :ALLOCATION :DYNAMIC)")
   ...
   (DEFINE-FUNCTION-OBJECT "FIND-SELECTION-KEY-WITH-EQUIVALENTS"
    "(DEFUN (FIND-SELECTION-KEY-WITH-EQUIVALENTS OBJECT) ((PATTERN SELECTION-PATTERN)))"
